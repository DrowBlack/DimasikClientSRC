...
extends WholeImageFilter {
    ...
    protected int[] filterPixels(int width, int height, int[] inPixels, Rectangle transformedSpace) {
        ...
        while (y < height) {
            ...
            int x2 = 0;
            while (x2 < width) {
                heightWindow[2][x2] = width45 * bump.evaluate(x2, y + 1);
                ++x2;
            }
            x2 = 0;
            while (x2 < width) {
                ...
                if (this.bumpSource != 3) {
                    ...
                    normal.y = 0.0f;
                    normal.x = 0.0f;
                    float m0 = heightWindow[1][x2];
                    float m1 = x0 ? heightWindow[1][x2 - 1] - m0 : 0.0f;
                    float m2 = y0 ? heightWindow[0][x2] - m0 : 0.0f;
                    float m3 = x1 ? heightWindow[1][x2 + 1] - m0 : 0.0f;
                    float f = m4 = y1 ? heightWindow[2][x2] - m0 : 0.0f;
                    if (x0 && y1) {
                        v1.x = -1.0f;
                        v1.y = 0.0f;
                        v1.z = m1;
    ...
    protected Color4f phongShade(Vector3f position, Vector3f viewpoint, Vector3f normal, Color4f diffuseColor, Color4f specularColor, Material material, Light[] lightsArray) {
        ...
        while (i < lightsArray.length) {
            ...
            if ((double)nDotL >= 0.0) {
                ...
                this.v.sub(position);
                this.v.normalize();
                if (light.type != 3 || !((dDotL = light.direction.dot(this.l)) < light.cosConeAngle)) {
