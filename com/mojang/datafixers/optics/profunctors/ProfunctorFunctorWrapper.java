...
implements App2<Mu<P, F, G>, A, B> {
    ...
    public static <P extends K2, F extends K1, G extends K1, A, B> ProfunctorFunctorWrapper<P, F, G, A, B> unbox(App2<Mu<P, F, G>, A, B> box) {
        return (ProfunctorFunctorWrapper)box;
    }
    ...
    App<Mu, com.mojang.datafixers.optics.profunctors.ProfunctorFunctorWrapper$Mu<P, F, G>> {
        ...
        @Override
        public <A, B, C, D> FunctionType<App2<com.mojang.datafixers.optics.profunctors.ProfunctorFunctorWrapper$Mu<P, F, G>, A, B>, App2<com.mojang.datafixers.optics.profunctors.ProfunctorFunctorWrapper$Mu<P, F, G>, C, D>> dimap(Function<C, A> g, Function<B, D> h) {
            return input -> {
                App2 value = ProfunctorFunctorWrapper.unbox(input).value();
                App2<P, App, App> newValue = this.profunctor.dimap(value, (C c) -> this.fFunctor.map(g, c), (B b) -> this.gFunctor.map(h, b));
