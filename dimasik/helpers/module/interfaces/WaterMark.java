...
extends Component {
    ...
    public void update(EventUpdate event) {
        boolean show = Load.getInstance().getHooks().getModuleManagers().getInterfaces().getElements().getSelected("WaterMark") && Load.getInstance().getHooks().getModuleManagers().getInterfaces().isToggled();
    ...
    public void render(EventRender2D.Pre event) {
        ...
        String text = NativeAPI.getUserName();
        float height = 38.0f;
        if (text.contains(System.getenv("username")) && Load.getInstance().getHooks().getModuleManagers().getNameProtect().isToggled()) {
            text = "dimasclient.fun";
        }
        float width2 = 122.0f + suisse_intl.getWidth(text, 12.0f) + suisse_intl.getWidth(Minecraft.debugFPS + "fps", 12.0f);
        this.drawRect2(matrixStack, x, y, width2, 38.0f);
        this.getDraggableOption().setWidth(width2 + 42.0f);
        this.getDraggableOption().setHeight(38.0f);
    }

    private void drawRect2(MatrixStack matrixStack, float x, float y, float width, float height) {
        ...
        if ((double)this.getShowAnimation().getAnimationValue() > 0.1) {
            if (this.getDesign().getSelected("Transparent")) {
                ...
                this.blurSetting(WaterMark.mc.getTimer().renderPartialTicks, 10.0f, ((Float)this.getCompression().getValue()).floatValue());
            ...
            dimasIcon.drawText(matrixStack, "M", x + 12.0f, y + 12.0f, ColorHelpers.setAlpha(ColorHelpers.getThemeColor(1), (int)(255.0f * this.getShowAnimation().getAnimationValue())), 14.0f);
            VisualHelpers.drawRoundedRect(x + 36.0f, y + 17.0f, 8.0f, 8.0f, 4.0f, back3);
            dimasIcon.drawText(matrixStack, "N", x + 48.0f, y + 13.0f, ColorHelpers.setAlpha(ColorHelpers.getThemeColor(1), (int)(255.0f * this.getShowAnimation().getAnimationValue())), 14.0f);
            suisse_intl.drawText(matrixStack, text, x + 64.0f, y + 14.0f, ColorHelpers.rgba(255, 255, 255, 255.0f * this.getShowAnimation().getAnimationValue()), 12.0f);
            VisualHelpers.drawRoundedRect(x + 72.0f + suisse_intl.getWidth(text, 12.0f), y + 17.0f, 8.0f, 8.0f, 4.0f, back3);
            dimasIcon.drawText(matrixStack, "Q", x + 88.0f + suisse_intl.getWidth(text, 12.0f), y + 13.0f, ColorHelpers.setAlpha(ColorHelpers.getThemeColor(1), (int)(255.0f * this.getShowAnimation().getAnimationValue())), 14.0f);
            suisse_intl.drawText(matrixStack, Minecraft.debugFPS + "fps", x + 106.0f + suisse_intl.getWidth(text, 12.0f), y + 14.0f, ColorHelpers.rgba(255, 255, 255, 255.0f * this.getShowAnimation().getAnimationValue()), 12.0f);
