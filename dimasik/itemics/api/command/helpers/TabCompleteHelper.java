package dimasik.itemics.api.command.helpers;
...
public class TabCompleteHelper {
    ...
    public TabCompleteHelper addSettings() {
        return this.append(ItemicsAPI.getSettings().allSettings.stream().filter((? super T s) -> !SettingsUtil.javaOnlySetting(s)).map(Settings.Setting::getName).sorted(String.CASE_INSENSITIVE_ORDER));
    }

    public TabCompleteHelper addModifiedSettings() {
        return this.append(SettingsUtil.modifiedSettings(ItemicsAPI.getSettings()).stream().map(Settings.Setting::getName).sorted(String.CASE_INSENSITIVE_ORDER));
    }

    public TabCompleteHelper addToggleableSettings() {
        return this.append(ItemicsAPI.getSettings().getAllValuesByType(Boolean.class).stream().map(Settings.Setting::getName).sorted(String.CASE_INSENSITIVE_ORDER));
    }
}
