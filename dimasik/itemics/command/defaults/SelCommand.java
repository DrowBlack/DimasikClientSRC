...
import dimasik.itemics.api.command.datatypes.ForDirection;
...
import dimasik.itemics.api.schematic.CompositeSchematic;
import dimasik.itemics.api.schematic.FillSchematic;
import dimasik.itemics.api.schematic.ISchematic;
import dimasik.itemics.api.schematic.ReplaceSchematic;
import dimasik.itemics.api.schematic.ShellSchematic;
import dimasik.itemics.api.schematic.WallsSchematic;
...
import dimasik.itemics.api.utils.BlockOptionalMetaLookup;
import dimasik.itemics.utils.BlockStateInterface;
import dimasik.itemics.utils.IRenderer;
import dimasik.itemics.utils.schematic.StaticSchematic;
...
extends Command {
    ...
    public SelCommand(IItemics itemics) {
        super(itemics, "sel", "selection", "s");
        this.manager = this.itemics.getSelectionManager();
        this.pos1 = null;
        this.clipboard = null;
        ...
        itemics.getGameEventHandler().registerEventListener(new AbstractGameEventListener(){
            ...
            public void onRenderPass(RenderEvent event) {
                if (!((Boolean)Itemics.settings().renderSelectionCorners.value).booleanValue() || SelCommand.this.pos1 == null) {
                    return;
                }
                Color color = (Color)Itemics.settings().colorSelectionPos1.value;
                float opacity = ((Float)Itemics.settings().selectionOpacity.value).floatValue();
                float lineWidth = ((Float)Itemics.settings().selectionLineWidth.value).floatValue();
                boolean ignoreDepth = (Boolean)Itemics.settings().renderSelectionIgnoreDepth.value;
                IRenderer.startLines(color, opacity, lineWidth, ignoreDepth);
                IRenderer.drawAABB(event.getModelViewStack(), new AxisAlignedBB(SelCommand.this.pos1, SelCommand.this.pos1.add(1, 1, 1)));
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (action == Action.POS1 || action == Action.POS2) {
            ...
            BetterBlockPos playerPos = mc.getRenderViewEntity() != null ? BetterBlockPos.from(mc.getRenderViewEntity().getPosition()) : this.ctx.playerFeet();
            ...
            this.itemics.getBuilderProcess().build("Fill", composite, (Vector3i)origin);
            ...
            for (ISelection selection : selections) {
                ...
                BetterBlockPos min = selection.min();
                final BlockState[][][] blockstates = new BlockState[size.getX()][size.getZ()][size.getY()];
                ...
                StaticSchematic schematic = new StaticSchematic(){
                    {
            ...
            this.itemics.getBuilderProcess().build("Fill", this.clipboard, (Vector3i)pos.add(this.clipboardOffset));
            ...
            Direction direction = (Direction)args.getDatatypeFor(ForDirection.INSTANCE);
