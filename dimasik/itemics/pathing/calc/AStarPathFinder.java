package dimasik.itemics.pathing.calc;
...
import dimasik.itemics.pathing.calc.Path;
import dimasik.itemics.pathing.calc.PathNode;
import dimasik.itemics.pathing.calc.openset.BinaryHeapOpenSet;
import dimasik.itemics.pathing.movement.CalculationContext;
import dimasik.itemics.pathing.movement.Moves;
import dimasik.itemics.utils.pathing.BetterWorldBorder;
import dimasik.itemics.utils.pathing.Favoring;
...
extends AbstractNodeCostSearch {
    ...
    protected Optional<IPath> calculate0(long primaryTimeout, long failureTimeout) {
        ...
        boolean slowPath = (Boolean)Itemics.settings().slowPath.value;
        if (slowPath) {
            this.logDebug("slowPath is on, path timeout will be " + String.valueOf(Itemics.settings().slowPathTimeoutMS.value) + "ms instead of " + primaryTimeout + "ms");
        }
        long primaryTimeoutTime = startTime + (slowPath ? (Long)Itemics.settings().slowPathTimeoutMS.value : primaryTimeout);
        long failureTimeoutTime = startTime + (slowPath ? (Long)Itemics.settings().slowPathTimeoutMS.value : failureTimeout);
        ...
        int pathingMaxChunkBorderFetch = (Integer)Itemics.settings().pathingMaxChunkBorderFetch.value;
        double minimumImprovement = (Boolean)Itemics.settings().minimumImprovementRepropagation.value != false ? 0.01 : 0.0;
        ...
        while (!(openSet.isEmpty() || numEmptyChunk >= pathingMaxChunkBorderFetch || this.cancelRequested || (numNodes & timeCheckInterval - 1) == 0 && ((now2 = System.currentTimeMillis()) - failureTimeoutTime >= 0L || !failing && now2 - primaryTimeoutTime >= 0L))) {
            ...
            if (slowPath) {
                try {
                    Thread.sleep((Long)Itemics.settings().slowPathTimeDelayMS.value);
                }
            ...
            for (Moves moves : allMoves) {
                ...
                if (actionCost <= 0.0 || Double.isNaN(actionCost)) {
                    throw new IllegalStateException(String.valueOf((Object)moves) + " calculated implausible cost " + actionCost);
                }
                if (moves.dynamicXZ && !worldBorder.entirelyContains(res.x, res.z)) continue;
                if (!(moves.dynamicXZ || res.x == newX && res.z == newZ)) {
                    throw new IllegalStateException(String.valueOf((Object)moves) + " " + res.x + " " + newX + " " + res.z + " " + newZ);
                }
                if (!moves.dynamicY && res.y != currentNode.y + moves.yOffset) {
                    throw new IllegalStateException(String.valueOf((Object)moves) + " " + res.y + " " + (currentNode.y + moves.yOffset));
                }
