...
import dimasik.itemics.api.pathing.calc.IPathFinder;
...
Helper {
    ...
    AbstractNodeCostSearch(int startX, int startY, int startZ, Goal goal, CalculationContext context) {
        ...
        this.map = new Long2ObjectOpenHashMap((Integer)Itemics.settings().pathingMapDefaultSize.value, ((Float)Itemics.settings().pathingMapLoadFactor.value).floatValue());
    }
    ...
    public synchronized PathCalculationResult calculate(long primaryTimeout, long failureTimeout) {
        ...
        try {
            ...
            Helper.HELPER.logDirect("Pathing exception: " + String.valueOf(e));
            e.printStackTrace();
    ...
    protected PathNode getNodeAtPosition(int x, int y, int z, long hashCode) {
        PathNode node = this.map.get(hashCode);
        if (node == null) {
