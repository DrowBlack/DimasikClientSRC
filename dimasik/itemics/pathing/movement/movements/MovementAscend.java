...
extends Movement {
    ...
    public MovementAscend(IItemics itemics, BetterBlockPos src, BetterBlockPos dest) {
        super(itemics, src, dest, new BetterBlockPos[]{dest, src.up(2), dest.up()}, dest.down());
    }
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (!MovementHelper.canWalkOn(this.ctx, this.positionToPlace, jumpingOnto)) {
            ...
            if (MovementHelper.attemptToPlaceABlock(state, this.itemics, this.dest.down(), false, true) == MovementHelper.PlaceResult.READY_TO_PLACE) {
        ...
        if (((Boolean)Itemics.settings().assumeStep.value).booleanValue() || this.ctx.playerFeet().equals(this.src.up())) {
            return state;
        }
        int xAxis = Math.abs(this.src.getX() - this.dest.getX());
        int zAxis = Math.abs(this.src.getZ() - this.dest.getZ());
        double flatDistToNext = (double)xAxis * Math.abs((double)this.dest.getX() + 0.5 - this.ctx.player().getPositionVec().x) + (double)zAxis * Math.abs((double)this.dest.getZ() + 0.5 - this.ctx.player().getPositionVec().z);
        double sideDist = (double)zAxis * Math.abs((double)this.dest.getX() + 0.5 - this.ctx.player().getPositionVec().x) + (double)xAxis * Math.abs((double)this.dest.getZ() + 0.5 - this.ctx.player().getPositionVec().z);
        double lateralMotion = (double)xAxis * this.ctx.player().getMotion().z + (double)zAxis * this.ctx.player().getMotion().x;
