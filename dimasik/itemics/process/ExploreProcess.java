...
import dimasik.itemics.api.utils.MyChunkPos;
...
implements IExploreProcess {
    ...
    public ExploreProcess(Itemics itemics) {
        super(itemics);
    }
    ...
    public IChunkFilter calcFilter() {
        IChunkFilter filter = this.filter != null ? new EitherChunk(this.filter, new ItemicsChunkCache()) : new ItemicsChunkCache();
        return filter;
    }
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        if (calcFailed) {
            ...
            if (((Boolean)Itemics.settings().notificationOnExploreFinished.value).booleanValue()) {
        ...
        if (!((Boolean)Itemics.settings().disableCompletionCheck.value).booleanValue() && filter.countRemain() == 0) {
    ...
    private Goal[] closestUncachedChunks(BlockPos center, IChunkFilter filter) {
        ...
        int count = Math.min(filter.countRemain(), (Integer)Itemics.settings().exploreChunkSetMinimumSize.value);
        ArrayList<BlockPos> centers = new ArrayList<BlockPos>();
        int renderDistance = (Integer)Itemics.settings().worldExploringChunkOffset.value;
        ...
        block5: while (true) {
            ...
            while (true) {
                ...
                if (dx <= dist) {
                    ...
                    if (dist % 10 == 0) {
                        count = Math.min(filter.countRemain(), (Integer)Itemics.settings().exploreChunkSetMinimumSize.value);
                    }
    ...
    private static Goal createGoal(int x, int z) {
        if ((Integer)Itemics.settings().exploreMaintainY.value == -1) {
        ...
        return new GoalXZ(x, z){
            ...
            public double heuristic(int x, int y, int z) {
                return super.heuristic(x, y, z) + GoalYLevel.calculate((Integer)Itemics.settings().exploreMaintainY.value, y);
            }
        };
    }
    ...
    implements IChunkFilter {
        ...
        public int countRemain() {
            ...
            int countRemain = 0;
            ItemicsChunkCache bcc = new ItemicsChunkCache();
            for (MyChunkPos pos : this.positions) {
                if (bcc.isAlreadyExplored(pos.x, pos.z) == Status.EXPLORED || ++countRemain < (Integer)Itemics.settings().exploreChunkSetMinimumSize.value) continue;
                return countRemain;
            }
            return countRemain;
        }
    ...
    private class ItemicsChunkCache
    implements IChunkFilter {
        private final ICachedWorld cache;

        private ItemicsChunkCache() {
            this.cache = ExploreProcess.this.itemics.getWorldProvider().getCurrentWorld().getCachedWorld();
        }
        ...
        public Status isAlreadyExplored(int chunkX, int chunkZ) {
            ...
            if (!((CachedWorld)this.cache).regionLoaded(centerX, centerZ)) {
                Itemics.getExecutor().execute(() -> ((CachedWorld)this.cache).tryLoadFromDisk(centerX >> 9, centerZ >> 9));
                return Status.UNKNOWN;
            }
