package dimasik.itemics.utils.schematic.format.defaults;

import dimasik.itemics.utils.schematic.StaticSchematic;
import dimasik.itemics.utils.type.VarInt;
...
extends StaticSchematic {
    public SpongeSchematic(CompoundNBT nbt) {
        ...
        Int2ObjectArrayMap<BlockState> palette = new Int2ObjectArrayMap<BlockState>();
        ...
        for (int y = 0; y < this.y; ++y) {
            for (int z = 0; z < this.z; ++z) {
                for (int x = 0; x < this.x; ++x) {
                    ...
                    BlockState state = (BlockState)palette.get(blockData[index]);
                    if (state == null) {
    ...
    private static final class SerializedBlockState {
        ...
        private static <T extends Comparable<T>> BlockState setPropertyValue(BlockState state, Property<T> property, String value) {
            ...
            if (parsed.isPresent()) {
                return (BlockState)state.with(property, (Comparable)parsed.get());
            }
