...
class CeltPitchXCorr {
    ...
    static int pitch_xcorr(int[] _x, int[] _y, int[] xcorr, int len, int max_pitch) {
        ...
        while (i < max_pitch) {
            int inner_sum;
            xcorr[i] = inner_sum = Kernels.celt_inner_prod(_x, 0, _y, i, len);
    ...
    static int pitch_xcorr(short[] _x, int _x_ptr, short[] _y, int _y_ptr, int[] xcorr, int len, int max_pitch) {
        ...
        while (i < max_pitch) {
            int inner_sum;
            xcorr[i] = inner_sum = Kernels.celt_inner_prod(_x, _x_ptr, _y, _y_ptr + i, len);
            maxcorr = Inlines.MAX32(maxcorr, inner_sum);
            ++i;
        }
        return maxcorr;
    ...
    static int pitch_xcorr(short[] _x, short[] _y, int[] xcorr, int len, int max_pitch) {
        ...
        while (i < max_pitch) {
            int inner_sum;
            xcorr[i] = inner_sum = Kernels.celt_inner_prod(_x, _y, i, len);
            maxcorr = Inlines.MAX32(maxcorr, inner_sum);
            ++i;
        }
        return maxcorr;
