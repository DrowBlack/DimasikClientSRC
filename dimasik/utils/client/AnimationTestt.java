...
public abstract class AnimationTestt {
    public TimerUtils timerUtil = new TimerUtils();
    protected int duration;
    protected double endPoint;
    protected Direction direction;

    public AnimationTestt(int ms, double endPoint) {
        this.duration = ms;
        this.endPoint = endPoint;
        this.direction = Direction.FORWARDS;
    }

    public AnimationTestt(int ms, double endPoint, Direction direction) {
        this.duration = ms;
        this.endPoint = endPoint;
    ...
    public AnimationTestt setDirection(boolean forwards) {
        Direction direction;
        Direction direction2 = direction = forwards ? Direction.FORWARDS : Direction.BACKWARDS;
        if (this.direction != direction) {
            this.direction = direction;
            this.timerUtil.setTime(System.currentTimeMillis() - ((long)this.duration - Math.min((long)this.duration, this.timerUtil.getTime())));
        }
