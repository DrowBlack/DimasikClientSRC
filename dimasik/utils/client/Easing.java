...
public enum Easing {
    ...
    LINEAR(x -> x),
    EASE_OUT_EXPO(x -> x.equals(1.0) ? 1.0 : 1.0 - Math.pow(2.0, -10.0 * x)),
    EASE_OUT_CUBIC(x -> {
        double adjustedX = x - 1.0;
        return adjustedX * adjustedX * adjustedX + 1.0;
    }),
    CIRC_OUT(x -> Math.sqrt(1.0 - Math.pow(x - 1.0, 2.0)));
    ...
    public static interface Easing2 {
