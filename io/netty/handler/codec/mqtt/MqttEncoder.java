...
extends MessageToMessageEncoder<MqttMessage> {
    ...
    private static ByteBuf encodeConnectMessage(ByteBufAllocator byteBufAllocator, MqttConnectMessage message) {
        ...
        byte[] byArray2 = userNameBytes = (userName = payload.userName()) != null ? MqttEncoder.encodeStringUtf8(userName) : EmptyArrays.EMPTY_BYTES;
        if (variableHeader.hasUserName()) {
            payloadBufferSize += 2 + userNameBytes.length;
        }
        byte[] byArray3 = passwordBytes = (password = payload.passwordInBytes()) != null ? password : EmptyArrays.EMPTY_BYTES;
