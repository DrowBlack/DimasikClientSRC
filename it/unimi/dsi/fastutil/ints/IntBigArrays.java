...
public final class IntBigArrays {
    ...
    public static void radixSort(int[][] a, long from, long to) {
        ...
        while (offsetPos > 0) {
            ...
            long i = length;
            while (i-- != 0L) {
                ByteBigArrays.set(digit, i, (byte)(IntBigArrays.get(a, first + i) >>> shift & 0xFF ^ signMask));
            }
            i = length;
            while (i-- != 0L) {
    ...
    public static void radixSort(int[][] a, int[][] b, long from, long to) {
        ...
        while (offsetPos > 0) {
            ...
            long i = length;
            while (i-- != 0L) {
                ByteBigArrays.set(digit, i, (byte)(IntBigArrays.get(k, first + i) >>> shift & 0xFF ^ signMask));
            }
            i = length;
            while (i-- != 0L) {
                int n2 = ByteBigArrays.get(digit, i) & 0xFF;
