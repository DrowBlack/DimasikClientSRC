...
implements Comparable<AdvancementProgress> {
    ...
    public void update(Map<String, Criterion> criteriaIn, String[][] requirements) {
        ...
        this.criteria.entrySet().removeIf(criteriaEntry -> !set.contains(criteriaEntry.getKey()));
        for (String s : set) {
            if (this.criteria.containsKey(s)) continue;
    ...
    public String toString() {
        return "AdvancementProgress{criteria=" + String.valueOf(this.criteria) + ", requirements=" + Arrays.deepToString((Object[])this.requirements) + "}";
    }
    ...
    public static AdvancementProgress fromNetwork(PacketBuffer buffer) {
        ...
        for (int j = 0; j < i; ++j) {
            advancementprogress.criteria.put(buffer.readString(Short.MAX_VALUE), CriterionProgress.read(buffer));
        }
    ...
    JsonSerializer<AdvancementProgress> {
        ...
        public JsonElement serialize(AdvancementProgress p_serialize_1_, Type p_serialize_2_, JsonSerializationContext p_serialize_3_) {
            ...
            for (Map.Entry<String, CriterionProgress> entry : p_serialize_1_.criteria.entrySet()) {
