...
public class ItemPredicate {
    ...
    public boolean test(ItemStack item) {
        ...
        if (!this.durability.isUnbounded() && !item.isDamageable()) {
            return false;
        }
        if (!this.durability.test(item.getMaxDamage() - item.getDamage())) {
            return false;
        }
        ...
        if (this.bookEnchantments.length > 0) {
            ...
            for (EnchantmentPredicate enchantmentpredicate1 : this.bookEnchantments) {
                if (enchantmentpredicate1.test(map1)) continue;
                return false;
            }
        }
    ...
    public static ItemPredicate deserialize(@Nullable JsonElement element) {
        if (element != null && !element.isJsonNull()) {
            ...
            if (jsonobject.has("item")) {
                ...
                item = Registry.ITEM.getOptional(resourcelocation).orElseThrow(() -> new JsonSyntaxException("Unknown item id '" + String.valueOf(resourcelocation) + "'"));
            }
            ITag<Item> itag = null;
            ...
            if (jsonobject.has("potion")) {
                ...
                potion = Registry.POTION.getOptional(resourcelocation2).orElseThrow(() -> new JsonSyntaxException("Unknown potion '" + String.valueOf(resourcelocation2) + "'"));
            }
