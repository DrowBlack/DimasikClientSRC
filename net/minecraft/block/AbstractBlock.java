...
public abstract class AbstractBlock {
    ...
    public VoxelShape getCollisionShape(BlockState state, IBlockReader worldIn, BlockPos pos, ISelectionContext context) {
        EventCollision event = new EventCollision(pos);
        Load.getInstance().getEvents().call(event);
        return !event.isCancelled() && this.canCollide ? state.getShape(worldIn, pos) : VoxelShapes.empty();
    }
    ...
    public static class Properties {
        ...
        private ToIntFunction<BlockState> lightLevel = light -> 0;
        ...
        private IPositionPredicate blocksVision = this.suffocates = (state, reader, pos) -> this.material.blocksMovement() && state.hasOpaqueCollisionShape(reader, pos);
        ...
        private Properties(Material materialIn, MaterialColor mapColorIn) {
            this(materialIn, (BlockState state) -> mapColorIn);
        }
        ...
        public static Properties create(Material materialIn) {
            return Properties.create(materialIn, materialIn.getColor());
        }

        public static Properties create(Material materialIn, DyeColor color) {
            return Properties.create(materialIn, color.getMapColor());
        }
    ...
    extends StateHolder<Block, BlockState> {
        ...
        public boolean causesSuffocation(IBlockReader worldIn, BlockPos pos) {
            EventGameOverlay eventGameOverlay = new EventGameOverlay(EventGameOverlay.OverlayType.Block);
        ...
        static final class Cache {
            ...
            private Cache(BlockState stateIn) {
                ...
                this.isCollisionShapeLargerThanFullBlock = Arrays.stream(Direction.Axis.values()).anyMatch(axis -> this.collisionShape.getStart((Direction.Axis)axis) < 0.0 || this.collisionShape.getEnd((Direction.Axis)axis) > 1.0);
                ...
                for (Direction direction1 : DIRECTIONS) {
                    for (BlockVoxelShape blockvoxelshape : BlockVoxelShape.values()) {
                        this.solidSides[Cache.func_242701_b((Direction)direction1, (BlockVoxelShape)blockvoxelshape)] = blockvoxelshape.func_241854_a(stateIn, EmptyBlockReader.INSTANCE, BlockPos.ZERO, direction1);
                    }
                }
