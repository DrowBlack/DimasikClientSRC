...
extends Block {
    ...
    public static boolean isRail(World worldIn, BlockPos pos) {
        return AbstractRailBlock.isRail(worldIn.getBlockState(pos));
    }
    ...
    public boolean isValidPosition(BlockState state, IWorldReader worldIn, BlockPos pos) {
        return AbstractRailBlock.hasSolidSideOnTop(worldIn, pos.down());
    }
    ...
    public void neighborChanged(BlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos, boolean isMoving) {
        if (!worldIn.isRemote && worldIn.getBlockState(pos).isIn(this)) {
            ...
            if (AbstractRailBlock.isValidRailDirection(pos, worldIn, railshape)) {
                AbstractRailBlock.spawnDrops(state, worldIn, pos);
    ...
    private static boolean isValidRailDirection(BlockPos pos, World world, RailShape railShape) {
        if (!AbstractRailBlock.hasSolidSideOnTop(world, pos.down())) {
            return true;
        }
        switch (railShape) {
            case ASCENDING_EAST: {
                return !AbstractRailBlock.hasSolidSideOnTop(world, pos.east());
            }
            case ASCENDING_WEST: {
                return !AbstractRailBlock.hasSolidSideOnTop(world, pos.west());
            }
            case ASCENDING_NORTH: {
                return !AbstractRailBlock.hasSolidSideOnTop(world, pos.north());
            }
            case ASCENDING_SOUTH: {
                return !AbstractRailBlock.hasSolidSideOnTop(world, pos.south());
            }
        }
        return false;
    }
