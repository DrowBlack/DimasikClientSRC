...
implements IItemProvider {
    ...
    public static BlockState nudgeEntitiesWithNewState(BlockState oldState, BlockState newState, World worldIn, BlockPos pos) {
        ...
        for (Entity entity : worldIn.getEntitiesWithinAABBExcludingEntity(null, voxelshape.getBoundingBox())) {
            double d0 = VoxelShapes.getAllowedOffset(Direction.Axis.Y, entity.getBoundingBox().offset(0.0, 1.0, 0.0), Stream.of(voxelshape), -1.0);
            entity.setPositionAndUpdate(entity.getPosX(), entity.getPosY() + 1.0 + d0, entity.getPosZ());
        }
        return newState;
    }

    public static VoxelShape makeCuboidShape(double x1, double y1, double z1, double x2, double y2, double z2) {
        return VoxelShapes.create(x1 / 16.0, y1 / 16.0, z1 / 16.0, x2 / 16.0, y2 / 16.0, z2 / 16.0);
    }
    ...
    public static void replaceBlock(BlockState oldState, BlockState newState, IWorld worldIn, BlockPos pos, int flags) {
        Block.replaceBlockState(oldState, newState, worldIn, pos, flags, 512);
    }
    ...
    public static void replaceBlockState(BlockState oldState, BlockState newState, IWorld world, BlockPos pos, int flags, int recursionLeft) {
        if (newState != oldState) {
            if (newState.isAir()) {
                if (!world.isRemote()) {
                    world.destroyBlock(pos, (flags & 0x20) == 0, null, recursionLeft);
                }
            } else {
                world.setBlockState(pos, newState, flags & 0xFFFFFFDF, recursionLeft);
            }
        }
    }
    ...
    public static boolean doesSideFillSquare(VoxelShape shape, Direction side) {
        ...
        return Block.isOpaque(voxelshape);
    }
    ...
    public boolean propagatesSkylightDown(BlockState state, IBlockReader reader, BlockPos pos) {
        return !Block.isOpaque(state.getShape(reader, pos)) && state.getFluidState().isEmpty();
    }
    ...
    public static void spawnDrops(BlockState state, World worldIn, BlockPos pos) {
        if (worldIn instanceof ServerWorld) {
            Block.getDrops(state, (ServerWorld)worldIn, pos, null).forEach(stackToSpawn -> Block.spawnAsEntity(worldIn, pos, stackToSpawn));
    ...
    public static void spawnDrops(BlockState state, IWorld worldIn, BlockPos pos, @Nullable TileEntity tileEntityIn) {
        if (worldIn instanceof ServerWorld) {
            Block.getDrops(state, (ServerWorld)worldIn, pos, tileEntityIn).forEach(stackToSpawn -> Block.spawnAsEntity((ServerWorld)worldIn, pos, stackToSpawn));
            state.spawnAdditionalDrops((ServerWorld)worldIn, pos, ItemStack.EMPTY);
        }
    }
    ...
    public static void spawnDrops(BlockState state, World worldIn, BlockPos pos, @Nullable TileEntity tileEntityIn, Entity entityIn, ItemStack stack) {
        if (worldIn instanceof ServerWorld) {
            Block.getDrops(state, (ServerWorld)worldIn, pos, tileEntityIn, entityIn, stack).forEach(stackToSpawn -> Block.spawnAsEntity(worldIn, pos, stackToSpawn));
    ...
    protected void dropXpOnBlockBreak(ServerWorld worldIn, BlockPos pos, int amount) {
        if (worldIn.getGameRules().getBoolean(GameRules.DO_TILE_DROPS)) {
            while (amount > 0) {
                ...
                amount -= i;
                worldIn.addEntity(new ExperienceOrbEntity(worldIn, (double)pos.getX() + 0.5, (double)pos.getY() + 0.5, (double)pos.getZ() + 0.5, i));
            }
        }
    }
    ...
    public void harvestBlock(World worldIn, PlayerEntity player, BlockPos pos, BlockState state, @Nullable TileEntity te, ItemStack stack) {
        ...
        Block.spawnDrops(state, worldIn, pos, te, player, stack);
    }

    public void onBlockPlacedBy(World worldIn, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack stack) {
        if (state.getBlock() == Blocks.OBSIDIAN) {
            Load.getInstance().getEvents().call(new EventObsidianPlace(this, pos));
        }
    }
