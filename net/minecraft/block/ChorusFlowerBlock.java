...
extends Block {
    ...
    public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random random) {
        ...
        if (worldIn.isAirBlock(blockpos) && blockpos.getY() < 256 && (i = state.get(AGE).intValue()) < 5) {
            boolean flag = false;
            boolean flag1 = false;
            ...
            if (flag && ChorusFlowerBlock.areAllNeighborsEmpty(worldIn, blockpos, null) && worldIn.isAirBlock(pos.up(2))) {
                ...
                for (int i1 = 0; i1 < l; ++i1) {
                    ...
                    if (!worldIn.isAirBlock(blockpos1) || !worldIn.isAirBlock(blockpos1.down()) || !ChorusFlowerBlock.areAllNeighborsEmpty(worldIn, blockpos1, direction.getOpposite())) continue;
    ...
    private void placeGrownFlower(World worldIn, BlockPos pos, int age) {
        worldIn.setBlockState(pos, (BlockState)this.getDefaultState().with(AGE, age), 2);
        worldIn.playEvent(1033, pos, 0);
    }

    private void placeDeadFlower(World worldIn, BlockPos pos) {
        worldIn.setBlockState(pos, (BlockState)this.getDefaultState().with(AGE, 5), 2);
    ...
    private static boolean areAllNeighborsEmpty(IWorldReader worldIn, BlockPos pos, @Nullable Direction excludingSide) {
        for (Direction direction : Direction.Plane.HORIZONTAL) {
            if (direction == excludingSide || worldIn.isAirBlock(pos.offset(direction))) continue;
            return false;
        }
        return true;
    }
    ...
    public static void generatePlant(IWorld worldIn, BlockPos pos, Random rand, int maxHorizontalDistance) {
        ...
        ChorusFlowerBlock.growTreeRecursive(worldIn, pos, rand, pos, maxHorizontalDistance, 0);
    }
    ...
    private static void growTreeRecursive(IWorld worldIn, BlockPos branchPos, Random rand, BlockPos originalBranchPos, int maxHorizontalDistance, int iterations) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            if (!ChorusFlowerBlock.areAllNeighborsEmpty(worldIn, blockpos, null)) {
                return;
            }
        ...
        if (iterations < 4) {
            ...
            for (int k = 0; k < l; ++k) {
                ...
                if (Math.abs(blockpos1.getX() - originalBranchPos.getX()) >= maxHorizontalDistance || Math.abs(blockpos1.getZ() - originalBranchPos.getZ()) >= maxHorizontalDistance || !worldIn.isAirBlock(blockpos1) || !worldIn.isAirBlock(blockpos1.down()) || !ChorusFlowerBlock.areAllNeighborsEmpty(worldIn, blockpos1, direction.getOpposite())) continue;
                flag = true;
                ...
                ChorusFlowerBlock.growTreeRecursive(worldIn, blockpos1, rand, originalBranchPos, maxHorizontalDistance, iterations + 1);
            }
        }
        if (!flag) {
            worldIn.setBlockState(branchPos.up(i), (BlockState)Blocks.CHORUS_FLOWER.getDefaultState().with(AGE, 5), 2);
        }
    }
