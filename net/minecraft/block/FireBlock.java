...
extends AbstractFireBlock {
    ...
    private static final Map<Direction, BooleanProperty> FACING_TO_PROPERTY_MAP = SixWayBlock.FACING_TO_PROPERTY_MAP.entrySet().stream().filter(facingProperty -> facingProperty.getKey() != Direction.DOWN).collect(Util.toMapCollector());
    ...
    public FireBlock(AbstractBlock.Properties builder) {
        ...
        this.stateToShapeMap = ImmutableMap.copyOf(this.stateContainer.getValidStates().stream().filter(state -> state.get(AGE) == 0).collect(Collectors.toMap(Function.identity(), FireBlock::getShapeForState)));
    }
    ...
    protected BlockState getStateForPlacement(IBlockReader blockReader, BlockPos pos) {
        ...
        if (!this.canBurn(blockstate) && !blockstate.isSolidSide(blockReader, blockpos, Direction.UP)) {
            ...
            for (Direction direction : Direction.values()) {
                ...
                blockstate1 = (BlockState)blockstate1.with(booleanproperty, this.canBurn(blockReader.getBlockState(pos.offset(direction))));
            }
            return blockstate1;
        }
    ...
    public void tick(BlockState state, ServerWorld worldIn, BlockPos pos, Random rand) {
        worldIn.getPendingBlockTicks().scheduleTick(pos, this, FireBlock.getTickCooldown(worldIn.rand));
        if (worldIn.getGameRules().getBoolean(GameRules.DO_FIRE_TICK)) {
            ...
            if (!flag && worldIn.isRaining() && this.canDie(worldIn, pos) && rand.nextFloat() < 0.2f + (float)i * 0.03f) {
                ...
                int k = (flag1 = worldIn.isBlockinHighHumidity(pos)) ? -50 : 0;
                ...
                for (int l = -1; l <= 1; ++l) {
                    for (int i1 = -1; i1 <= 1; ++i1) {
                        for (int j1 = -1; j1 <= 4; ++j1) {
                            ...
                            if (i2 <= 0 || rand.nextInt(k1) > i2 || worldIn.isRaining() && this.canDie(worldIn, blockpos$mutable)) continue;
    ...
    private int getFlammability(BlockState state) {
        return state.hasProperty(BlockStateProperties.WATERLOGGED) && state.get(BlockStateProperties.WATERLOGGED) != false ? 0 : this.flammabilities.getInt(state.getBlock());
    }

    private int getFireSpreadSpeed(BlockState state) {
        return state.hasProperty(BlockStateProperties.WATERLOGGED) && state.get(BlockStateProperties.WATERLOGGED) != false ? 0 : this.encouragements.getInt(state.getBlock());
    }
    ...
    private BlockState getFireWithAge(IWorld world, BlockPos pos, int age) {
        BlockState blockstate = FireBlock.getFireForPlacement(world, pos);
        return blockstate.isIn(Blocks.FIRE) ? (BlockState)blockstate.with(AGE, age) : blockstate;
    }

    private boolean areNeighborsFlammable(IBlockReader worldIn, BlockPos pos) {
        for (Direction direction : Direction.values()) {
            if (!this.canBurn(worldIn.getBlockState(pos.offset(direction)))) continue;
            return true;
        }
        return false;
    }
