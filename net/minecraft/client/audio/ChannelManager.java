...
public class ChannelManager {
    ...
    public void runForAllSoundSources(Consumer<Stream<SoundSource>> sourceStreamConsumer) {
        this.soundExecutor.execute(() -> sourceStreamConsumer.accept(this.channels.stream().map(managerEntry -> managerEntry.source).filter(Objects::nonNull)));
    }
    ...
    public void tick() {
        this.soundExecutor.execute(() -> {
            ...
            while (iterator.hasNext()) {
                ...
                if (!channelmanager$entry.source.isStopped()) continue;
