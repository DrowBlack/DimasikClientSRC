...
public class RecipeList {
    ...
    public RecipeList(List<IRecipe<?>> p_i242062_1_) {
        ...
        this.singleResultItem = p_i242062_1_.size() <= 1 ? true : RecipeList.func_243413_a(p_i242062_1_);
    }
    ...
    private static boolean func_243413_a(List<IRecipe<?>> p_243413_0_) {
        ...
        for (int j = 1; j < i; ++j) {
            ...
            if (ItemStack.areItemsEqual(itemstack, itemstack1) && ItemStack.areItemStackTagsEqual(itemstack, itemstack1)) continue;
            return false;
        }
        return true;
    }
    ...
    public void updateKnownRecipes(RecipeBook book) {
        for (IRecipe<?> irecipe : this.recipes) {
            if (!book.isUnlocked(irecipe)) continue;
    ...
    public void canCraft(RecipeItemHelper handler, int width, int height, RecipeBook book) {
        for (IRecipe<?> irecipe : this.recipes) {
            boolean flag;
            boolean bl = flag = irecipe.canFit(width, height) && book.isUnlocked(irecipe);
            if (flag) {
                this.canFit.add(irecipe);
            } else {
                this.canFit.remove(irecipe);
            }
            if (flag && handler.canCraft(irecipe, null)) {
    ...
    public List<IRecipe<?>> getDisplayRecipes(boolean onlyCraftable) {
        ArrayList<IRecipe<?>> list = Lists.newArrayList();
        for (IRecipe<?> irecipe : this.recipes) {
            if (!this.canFit.contains(irecipe) || this.craftable.contains(irecipe) != onlyCraftable) continue;
            list.add(irecipe);
        }
        return list;
    }
