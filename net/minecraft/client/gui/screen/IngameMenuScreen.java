...
extends Screen {
    ...
    private void addButtons() {
        ...
        if (!ClientManagers.isUnHook()) {
            this.addButton(new Button(this.width / 2 - 102, this.height / 4 + 144 + -16, 204, 20, new TranslationTextComponent("Reconnect"), button2 -> this.reconnectToServer()));
        }
        ...
        this.addButton(new Button(this.width / 2 - 102, this.height / 4 + 48 + -16, 98, 20, new TranslationTextComponent("gui.advancements"), button2 -> this.minecraft.displayGuiScreen(new AdvancementsScreen(this.minecraft.player.connection.getAdvancementManager()))));
        this.addButton(new Button(this.width / 2 + 4, this.height / 4 + 48 + -16, 98, 20, new TranslationTextComponent("gui.stats"), button2 -> this.minecraft.displayGuiScreen(new StatsScreen(this, this.minecraft.player.getStats()))));
        String s = SharedConstants.getVersion().isStable() ? "https://aka.ms/javafeedback?ref=game" : "https://aka.ms/snapshotfeedback?ref=game";
        this.addButton(new Button(this.width / 2 - 102, this.height / 4 + 72 + -16, 98, 20, new TranslationTextComponent("menu.sendFeedback"), button2 -> this.minecraft.displayGuiScreen(new ConfirmOpenLinkScreen(open -> {
            if (open) {
                Util.getOSType().openURI(s);
            }
            this.minecraft.displayGuiScreen(this);
        }, s, true))));
        this.addButton(new Button(this.width / 2 + 4, this.height / 4 + 72 + -16, 98, 20, new TranslationTextComponent("menu.reportBugs"), button2 -> this.minecraft.displayGuiScreen(new ConfirmOpenLinkScreen(open -> {
            if (open) {
                Util.getOSType().openURI("https://aka.ms/snapshotbugs?ref=game");
            }
            this.minecraft.displayGuiScreen(this);
        }, "https://aka.ms/snapshotbugs?ref=game", true))));
        this.addButton(new Button(this.width / 2 - 102, this.height / 4 + 96 + -16, 98, 20, new TranslationTextComponent("menu.options"), button2 -> this.minecraft.displayGuiScreen(new OptionsScreen(this, this.minecraft.gameSettings))));
        Button button = this.addButton(new Button(this.width / 2 + 4, this.height / 4 + 96 + -16, 98, 20, new TranslationTextComponent("menu.shareToLan"), button2 -> this.minecraft.displayGuiScreen(new ShareToLanScreen(this))));
