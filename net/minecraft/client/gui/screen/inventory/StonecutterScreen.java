...
extends ContainerScreen<StonecutterContainer> {
    ...
    protected void renderHoveredTooltip(MatrixStack matrixStack, int x, int y) {
        ...
        if (this.hasItemsInInputSlot) {
            ...
            for (int l = this.recipeIndexOffset; l < k && l < ((StonecutterContainer)this.container).getRecipeListSize(); ++l) {
    ...
    private void func_238853_b_(MatrixStack matrixStack, int x, int y, int p_238853_4_, int p_238853_5_, int p_238853_6_) {
        for (int i = this.recipeIndexOffset; i < p_238853_6_ && i < ((StonecutterContainer)this.container).getRecipeListSize(); ++i) {
    ...
    private void drawRecipesItems(int left, int top, int recipeIndexOffsetMax) {
        List<StonecuttingRecipe> list = ((StonecutterContainer)this.container).getRecipeList();
        for (int i = this.recipeIndexOffset; i < recipeIndexOffsetMax && i < ((StonecutterContainer)this.container).getRecipeListSize(); ++i) {
            int j = i - this.recipeIndexOffset;
            int k = left + j % 4 * 16;
            int l = j / 4;
            int i1 = top + l * 18 + 2;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (this.hasItemsInInputSlot) {
            ...
            for (int l = this.recipeIndexOffset; l < k; ++l) {
                ...
                if (!(d0 >= 0.0) || !(d1 >= 0.0) || !(d0 < 16.0) || !(d1 < 18.0) || !((StonecutterContainer)this.container).enchantItem(this.minecraft.player, l)) continue;
                Minecraft.getInstance().getSoundHandler().play(SimpleSound.master(SoundEvents.UI_STONECUTTER_SELECT_RECIPE, 1.0f));
                this.minecraft.playerController.sendEnchantPacket(((StonecutterContainer)this.container).windowId, l);
                return true;
            }
            i = this.guiLeft + 119;
            j = this.guiTop + 9;
