...
extends AbstractOptionList<Row> {
    ...
    public int addOption(AbstractOption p_214333_1_) {
        return this.addEntry(Row.create(this.minecraft.gameSettings, this.width, p_214333_1_));
    }
    ...
    public Widget func_243271_b(AbstractOption p_243271_1_) {
        for (Row optionsrowlist$row : this.getEventListeners()) {
            for (Widget widget : optionsrowlist$row.widgets) {
                if (!(widget instanceof OptionButton) || ((OptionButton)widget).func_238517_a_() != p_243271_1_) continue;
                return widget;
            }
        }
        return null;
    ...
    extends AbstractOptionList.Entry<Row> {
        ...
        public static Row create(GameSettings settings, int guiWidth, AbstractOption option) {
            return new Row(ImmutableList.of(option.createWidget(settings, guiWidth / 2 - 155, 0, 310)));
        }

        public static Row create(GameSettings settings, int guiWidth, AbstractOption leftOption, @Nullable AbstractOption rightOption) {
            Widget widget = leftOption.createWidget(settings, guiWidth / 2 - 155, 0, 150);
            return rightOption == null ? new Row(ImmutableList.of(widget)) : new Row(ImmutableList.of(widget, rightOption.createWidget(settings, guiWidth / 2 - 155 + 160, 0, 150)));
        }
