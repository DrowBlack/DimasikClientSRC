...
public class LanServerDetector {
    ...
    public static class LanServerList {
        ...
        public synchronized void addServer(String pingResponse, InetAddress ipAddress) {
            ...
            Object s1 = LanServerPingThread.getAdFromPingResponse(pingResponse);
            if (s1 != null) {
                s1 = ipAddress.getHostAddress() + ":" + (String)s1;
                boolean flag = false;
                for (LanServerInfo lanserverinfo : this.listOfLanServers) {
                    if (!lanserverinfo.getServerIpPort().equals(s1)) continue;
                    lanserverinfo.updateLastSeen();
                    flag = true;
                    break;
                }
                if (!flag) {
                    this.listOfLanServers.add(new LanServerInfo(s, (String)s1));
    ...
    extends Thread {
        ...
        public LanServerFindThread(LanServerList list) throws IOException {
