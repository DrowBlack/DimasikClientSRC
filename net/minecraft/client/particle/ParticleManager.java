...
implements IFutureReloadListener {
    ...
    public CompletableFuture<Void> reload(IFutureReloadListener.IStage stage, IResourceManager resourceManager, IProfiler preparationsProfiler, IProfiler reloadProfiler, Executor backgroundExecutor, Executor gameExecutor) {
        ...
        CompletableFuture[] completablefuture = (CompletableFuture[])Registry.PARTICLE_TYPE.keySet().stream().map(p_lambda$reload$1_4_ -> CompletableFuture.runAsync(() -> this.loadTextureLists(resourceManager, (ResourceLocation)p_lambda$reload$1_4_, map), backgroundExecutor)).toArray(CompletableFuture[]::new);
        return ((CompletableFuture)((CompletableFuture)CompletableFuture.allOf(completablefuture).thenApplyAsync(p_lambda$reload$3_4_ -> {
            ...
            this.atlas.upload((AtlasTexture.SheetData)p_lambda$reload$5_3_);
    ...
    private void loadTextureLists(IResourceManager manager, ResourceLocation particleId, Map<ResourceLocation, List<ResourceLocation>> textures) {
        ...
        try (IResource iresource = manager.getResource(resourcelocation);
            ...
            if (list == null) {
                ...
                textures.put(particleId, list.stream().map(p_lambda$loadTextureLists$6_0_ -> new ResourceLocation(p_lambda$loadTextureLists$6_0_.getNamespace(), "particle/" + p_lambda$loadTextureLists$6_0_.getPath())).collect(Collectors.toList()));
            }
        }
        catch (IOException ioexception1) {
            throw new IllegalStateException("Failed to load description for particle " + String.valueOf(particleId), ioexception1);
        }
    }
    ...
    private <T extends IParticleData> Particle makeParticle(T particleData, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
        IParticleFactory<?> iparticlefactory = this.factories.get(Registry.PARTICLE_TYPE.getKey(particleData.getType()));
    ...
    public void addEffect(Particle effect) {
        if (effect != null && (!(effect instanceof FireworkParticle.Spark) || Config.isFireworkParticles())) {
    ...
    public void tick() {
        this.byType.forEach((p_lambda$tick$7_1_, p_lambda$tick$7_2_) -> {
            ...
            this.tickParticleList((Collection<Particle>)p_lambda$tick$7_2_);
        ...
        if (!this.queue.isEmpty()) {
            ...
            while ((particle = this.queue.poll()) != null) {
                Queue queue = this.byType.computeIfAbsent(particle.getRenderType(), p_lambda$tick$8_0_ -> EvictingQueue.create(16384));
    ...
    private void tickParticleList(Collection<Particle> particlesIn) {
        if (!particlesIn.isEmpty()) {
            ...
            if (j > 0) {
                Iterator<Particle> iterator1 = particlesIn.iterator();
    ...
    public void renderParticles(MatrixStack p_renderParticles_1_, IRenderTypeBuffer.Impl p_renderParticles_2_, LightTexture p_renderParticles_3_, ActiveRenderInfo p_renderParticles_4_, float p_renderParticles_5_, ClippingHelper p_renderParticles_6_) {
        ...
        for (IParticleRenderType iparticlerendertype : collection) {
            ...
            for (Particle particle : iterable) {
                if (p_renderParticles_6_ != null && particle.shouldCull() && !p_renderParticles_6_.isBoundingBoxInFrustum(particle.getBoundingBox())) continue;
                try {
                    if (!flag && particle instanceof UnderwaterParticle && particle.motionX == 0.0 && particle.motionY == 0.0 && particle.motionZ == 0.0) continue;
                ...
                for (int l = 0; l < i; ++l) {
                    for (int i1 = 0; i1 < j; ++i1) {
                        for (int j1 = 0; j1 < k; ++j1) {
                            ...
                            this.addEffect(new DiggingParticle(this.world, (double)pos.getX() + d7, (double)pos.getY() + d8, (double)pos.getZ() + d9, d4 - 0.5, d5 - 0.5, d6 - 0.5, state).setBlockPos(pos));
                        }
                    }
                }
            });
    ...
    public void addBlockHitEffects(BlockPos pos, Direction side) {
        ...
        if (blockstate.getRenderType() != BlockRenderType.INVISIBLE) {
            ...
            this.addEffect(new DiggingParticle(this.world, d0, d1, d2, 0.0, 0.0, 0.0, blockstate).setBlockPos(pos).multiplyVelocity(0.2f).multiplyParticleScaleBy(0.6f));
        }
    }
    ...
    private boolean reuseBarrierParticle(Particle p_reuseBarrierParticle_1_, Queue<Particle> p_reuseBarrierParticle_2_) {
        for (Particle particle : p_reuseBarrierParticle_2_) {
            if (!(particle instanceof BarrierParticle) || p_reuseBarrierParticle_1_.prevPosX != particle.prevPosX || p_reuseBarrierParticle_1_.prevPosY != particle.prevPosY || p_reuseBarrierParticle_1_.prevPosZ != particle.prevPosZ) continue;
            particle.age = 0;
            return true;
        }
        return false;
    ...
    public void addBlockHitEffects(BlockPos p_addBlockHitEffects_1_, BlockRayTraceResult p_addBlockHitEffects_2_) {
        ...
        if (blockstate != null && !(flag = Reflector.callBoolean(blockstate, Reflector.IForgeBlockState_addHitEffects, this.world, p_addBlockHitEffects_2_, this))) {
