    ...
    public void reloadModels() {
        ...
        for (Block block : Registry.BLOCK) {
            block.getStateContainer().getValidStates().forEach(state -> {
                IBakedModel ibakedmodel = this.bakedModelStore.put((BlockState)state, this.modelManager.getModel(BlockModelShapes.getModelLocation(state)));
            });
        }
    }

    public static ModelResourceLocation getModelLocation(BlockState state) {
        return BlockModelShapes.getModelLocation(Registry.BLOCK.getKey(state.getBlock()), state);
    }

    public static ModelResourceLocation getModelLocation(ResourceLocation location, BlockState state) {
        return new ModelResourceLocation(location, BlockModelShapes.getPropertyMapString(state.getValues()));
    }

    public static String getPropertyMapString(Map<Property<?>, Comparable<?>> propertyValues) {
        StringBuilder stringbuilder = new StringBuilder();
        for (Map.Entry<Property<?>, Comparable<?>> entry : propertyValues.entrySet()) {
