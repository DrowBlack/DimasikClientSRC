...
implements IResourceManagerReloadListener {
    ...
    public ItemRenderer(TextureManager textureManagerIn, ModelManager modelManagerIn, ItemColors itemColorsIn) {
        ...
        this.itemModelMesher = Reflector.ItemModelMesherForge_Constructor.exists() ? (ItemModelMesher)Reflector.newInstance(Reflector.ItemModelMesherForge_Constructor, this.modelManager) : new ItemModelMesher(modelManagerIn);
    ...
    public void renderModel(IBakedModel modelIn, ItemStack stack, int combinedLightIn, int combinedOverlayIn, MatrixStack matrixStackIn, IVertexBuilder bufferIn) {
        ...
        for (Direction direction : Direction.VALUES) {
            random.setSeed(42L);
            this.renderQuads(matrixStackIn, bufferIn, modelIn.getQuads(null, direction, random), stack, combinedLightIn, combinedOverlayIn);
        }
        random.setSeed(42L);
        this.renderQuads(matrixStackIn, bufferIn, modelIn.getQuads(null, null, random), stack, combinedLightIn, combinedOverlayIn);
    }
    ...
    public void renderItem(ItemStack itemStackIn, ItemCameraTransforms.TransformType transformTypeIn, boolean leftHand, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn, IBakedModel modelIn) {
        if (!itemStackIn.isEmpty()) {
            ...
            if (Reflector.ForgeHooksClient_handleCameraTransforms.exists()) {
                modelIn = (IBakedModel)Reflector.ForgeHooksClient_handleCameraTransforms.call(new Object[]{matrixStackIn, modelIn, transformTypeIn, leftHand});
            } else {
            ...
            if (!modelIn.isBuiltInRenderer() && (itemStackIn.getItem() != Items.TRIDENT || flag)) {
                Block block;
                boolean flag1 = transformTypeIn != ItemCameraTransforms.TransformType.GUI && !transformTypeIn.isFirstPerson() && itemStackIn.getItem() instanceof BlockItem ? !((block = ((BlockItem)itemStackIn.getItem()).getBlock()) instanceof BreakableBlock) && !(block instanceof StainedGlassPaneBlock) : true;
                if (modelIn.isLayered()) {
                    ...
                    if (itemStackIn.getItem() == Items.COMPASS && itemStackIn.hasEffect()) {
                        ...
                        ivertexbuilder = flag1 ? ItemRenderer.getDirectGlintVertexBuilder(bufferIn, rendertype, matrixstack$entry) : ItemRenderer.getGlintVertexBuilder(bufferIn, rendertype, matrixstack$entry);
                        matrixStackIn.pop();
                    } else {
                        ivertexbuilder = flag1 ? ItemRenderer.getEntityGlintVertexBuilder(bufferIn, rendertype, true, itemStackIn.hasEffect()) : ItemRenderer.getBuffer(bufferIn, rendertype, true, itemStackIn.hasEffect());
                    }
                ...
                ItemStackTileEntityRenderer itemstacktileentityrenderer = (ItemStackTileEntityRenderer)Reflector.call(itemStackIn.getItem(), Reflector.IForgeItem_getItemStackTileEntityRenderer, new Object[0]);
    ...
    public IBakedModel getItemModelWithOverrides(ItemStack stack, @Nullable World worldIn, @Nullable LivingEntity entitylivingbaseIn) {
        ...
        IBakedModel ibakedmodel = item == Items.TRIDENT ? this.itemModelMesher.getModelManager().getModel(new ModelResourceLocation("minecraft:trident_in_hand#inventory")) : this.itemModelMesher.getItemModel(stack);
    ...
    protected void renderItemModelIntoGUI(ItemStack stack, int x, int y, IBakedModel bakedmodel) {
        ...
        RenderSystem.translatef(x, y, 100.0f + this.zLevel);
        RenderSystem.translatef(8.0f, 8.0f, 0.0f);
        RenderSystem.scalef(1.0f, -1.0f, 1.0f);
        RenderSystem.scalef(16.0f, 16.0f, 16.0f);
        MatrixStack matrixstack = new MatrixStack();
        IRenderTypeBuffer.Impl irendertypebuffer$impl = Minecraft.getInstance().getRenderTypeBuffers().getBufferSource();
        boolean bl = flag = !bakedmodel.isSideLit();
        if (flag) {
            RenderHelper.setupGuiFlatDiffuseLighting();
        }
        this.renderItem(stack, ItemCameraTransforms.TransformType.GUI, false, matrixstack, irendertypebuffer$impl, 0xF000F0, OverlayTexture.NO_OVERLAY, bakedmodel);
    ...
    public void renderModelItem(ItemStack stack, int x, int y) {
        renderItemGui = true;
        RenderSystem.pushMatrix();
        this.textureManager.bindTexture(AtlasTexture.LOCATION_BLOCKS_TEXTURE);
        this.textureManager.getTexture(AtlasTexture.LOCATION_BLOCKS_TEXTURE).setBlurMipmapDirect(false, false);
        ...
        MatrixStack matrixstack = new MatrixStack();
        IRenderTypeBuffer.Impl irendertypebuffer$impl = Minecraft.getInstance().getRenderTypeBuffers().getBufferSource();
        this.renderItem(stack, ItemCameraTransforms.TransformType.GUI, false, matrixstack, irendertypebuffer$impl, 0xF000F0, OverlayTexture.NO_OVERLAY, this.getItemModelWithOverrides(stack, null, null));
        irendertypebuffer$impl.finish();
        RenderSystem.enableDepthTest();
        RenderSystem.disableAlphaTest();
        RenderSystem.disableRescaleNormal();
    ...
    private void renderItemIntoGUI(@Nullable LivingEntity livingEntity, ItemStack stack, int x, int y) {
        if (!stack.isEmpty()) {
            ...
            try {
                ...
                crashreportcategory.addDetail("Registry Name", () -> String.valueOf(Reflector.call(stack.getItem(), Reflector.ForgeRegistryEntry_getRegistryName, new Object[0])));
    ...
    public void renderItemOverlayIntoGUI(FontRenderer fr, ItemStack stack, int xPosition, int yPosition, @Nullable String text) {
        if (!stack.isEmpty()) {
            ...
            if (stack.getCount() != 1 || text != null) {
                ...
                fr.renderString(s, xPosition + 19 - 2 - fr.getStringWidth(s), yPosition + 6 + 3, 0xFFFFFF, true, matrixstack.getLast().getMatrix(), irendertypebuffer$impl, false, 0, 0xF000F0);
                irendertypebuffer$impl.finish();
            }
            ...
            float f = f3 = (clientplayerentity = Minecraft.getInstance().player) == null ? 0.0f : clientplayerentity.getCooldownTracker().getCooldown(stack.getItem(), Minecraft.getInstance().getRenderPartialTicks());
            if (f3 > 0.0f) {
                RenderSystem.disableDepthTest();
                RenderSystem.disableTexture();
                RenderSystem.enableBlend();
