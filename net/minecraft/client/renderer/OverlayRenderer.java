...
public class OverlayRenderer {
    ...
    public static void renderOverlays(Minecraft minecraftIn, MatrixStack matrixStackIn) {
        ...
        ClientPlayerEntity playerentity = minecraftIn.player;
        if (!playerentity.noClip) {
            if (Reflector.ForgeEventFactory_renderBlockOverlay.exists() && Reflector.ForgeBlockModelShapes_getTexture3.exists()) {
                ...
                if (pair != null && !Reflector.ForgeEventFactory_renderBlockOverlay.callBoolean(playerentity, matrixStackIn, object = Reflector.getFieldValue(Reflector.RenderBlockOverlayEvent_OverlayType_BLOCK), pair.getLeft(), pair.getRight())) {
                    ...
                    OverlayRenderer.renderTexture(minecraftIn, textureatlassprite, matrixStackIn);
                }
            } else {
                BlockState blockstate = OverlayRenderer.getViewBlockingState(playerentity);
                if (blockstate != null) {
                    OverlayRenderer.renderTexture(minecraftIn, minecraftIn.getBlockRendererDispatcher().getBlockModelShapes().getTexture(blockstate), matrixStackIn);
                }
            }
        }
        if (!minecraftIn.player.isSpectator()) {
            if (minecraftIn.player.areEyesInFluid(FluidTags.WATER) && !Reflector.ForgeEventFactory_renderWaterOverlay.callBoolean(playerentity, matrixStackIn)) {
                OverlayRenderer.renderUnderwater(minecraftIn, matrixStackIn);
            }
            EventGameOverlay eventGameOverlay = new EventGameOverlay(EventGameOverlay.OverlayType.Fire);
            Load.getInstance().getEvents().call(eventGameOverlay);
            if (minecraftIn.player.isBurning() && !eventGameOverlay.isCancelled()) {
                OverlayRenderer.renderFire(minecraftIn, matrixStackIn);
            }
        }
    ...
    private static BlockState getViewBlockingState(PlayerEntity playerIn) {
        Pair<BlockState, BlockPos> pair = OverlayRenderer.getOverlayBlock(playerIn);
    ...
    private static Pair<BlockState, BlockPos> getOverlayBlock(PlayerEntity p_getOverlayBlock_0_) {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            if (blockstate.getRenderType() == BlockRenderType.INVISIBLE || !blockstate.causesSuffocation(p_getOverlayBlock_0_.world, blockpos$mutable)) continue;
    ...
    private static void renderFire(Minecraft minecraftIn, MatrixStack matrixStackIn) {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            float f15 = 0.5f;
            float f16 = -0.5f;
            matrixStackIn.translate((float)(-(i * 2 - 1)) * 0.24f, -0.3f, 0.0);
