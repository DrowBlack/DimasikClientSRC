...
extends LayerRenderer<IronGolemEntity, IronGolemModel<IronGolemEntity>> {
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, IronGolemEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (!entitylivingbaseIn.isInvisible() && (irongolementity$cracks = entitylivingbaseIn.func_226512_l_()) != IronGolemEntity.Cracks.NONE) {
            ResourceLocation resourcelocation = field_229134_a_.get((Object)irongolementity$cracks);
            IronGolemCracksLayer.renderCutoutModel(this.getEntityModel(), resourcelocation, matrixStackIn, bufferIn, packedLightIn, entitylivingbaseIn, 1.0f, 1.0f, 1.0f);
        }
    }
}
