...
extends LayerRenderer<T, M> {
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (((IEquipable)entitylivingbaseIn).isHorseSaddled()) {
            ((EntityModel)this.getEntityModel()).copyModelAttributesTo(this.field_239409_b_);
            ((EntityModel)this.field_239409_b_).setLivingAnimations(entitylivingbaseIn, limbSwing, limbSwingAmount, partialTicks);
            ((EntityModel)this.field_239409_b_).setRotationAngles(entitylivingbaseIn, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch);
            IVertexBuilder ivertexbuilder = bufferIn.getBuffer(RenderType.getEntityCutoutNoCull(this.field_239408_a_));
            ((Model)this.field_239409_b_).render(matrixStackIn, ivertexbuilder, packedLightIn, OverlayTexture.NO_OVERLAY, 1.0f, 1.0f, 1.0f, 1.0f);
        }
    }
}
