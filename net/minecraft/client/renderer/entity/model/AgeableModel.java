...
extends EntityModel<E> {
    ...
    public void render(MatrixStack matrixStackIn, IVertexBuilder bufferIn, int packedLightIn, int packedOverlayIn, float red, float green, float blue, float alpha) {
        if (this.isChild) {
            ...
            matrixStackIn.translate(0.0, this.childHeadOffsetY / 16.0f, this.childHeadOffsetZ / 16.0f);
            this.getHeadParts().forEach(p_228230_8_ -> p_228230_8_.render(matrixStackIn, bufferIn, packedLightIn, packedOverlayIn, red, green, blue, alpha));
            matrixStackIn.pop();
            matrixStackIn.push();
            float f1 = 1.0f / this.childBodyScale;
            matrixStackIn.scale(f1, f1, f1);
            matrixStackIn.translate(0.0, this.childBodyOffsetY / 16.0f, 0.0);
            this.getBodyParts().forEach(p_228229_8_ -> p_228229_8_.render(matrixStackIn, bufferIn, packedLightIn, packedOverlayIn, red, green, blue, alpha));
            matrixStackIn.pop();
        } else {
            this.getHeadParts().forEach(p_228228_8_ -> p_228228_8_.render(matrixStackIn, bufferIn, packedLightIn, packedOverlayIn, red, green, blue, alpha));
            this.getBodyParts().forEach(p_228227_8_ -> p_228227_8_.render(matrixStackIn, bufferIn, packedLightIn, packedOverlayIn, red, green, blue, alpha));
        }
    }
