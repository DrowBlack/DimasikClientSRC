...
extends SimpleTexture {
    ...
    private void setImageImpl(NativeImage p_setImageImpl_1_) {
        ...
        Minecraft.getInstance().execute(() -> {
            ...
            if (!RenderSystem.isOnRenderThread()) {
                RenderSystem.recordRenderCall(() -> this.upload(p_setImageImpl_1_));
            } else {
    ...
    private static NativeImage processLegacySkin(NativeImage nativeImageIn) {
        boolean flag;
        boolean bl = flag = nativeImageIn.getHeight() == 32;
        if (flag) {
        ...
        DownloadingTexture.setAreaOpaque(nativeImageIn, 0, 0, 32, 16);
        if (flag) {
            DownloadingTexture.setAreaTransparent(nativeImageIn, 32, 0, 64, 32);
        }
        DownloadingTexture.setAreaOpaque(nativeImageIn, 0, 16, 64, 32);
        DownloadingTexture.setAreaOpaque(nativeImageIn, 16, 48, 48, 64);
