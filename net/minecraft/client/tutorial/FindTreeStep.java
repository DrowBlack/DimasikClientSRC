...
implements ITutorialStep {
    ...
    public void onMouseHover(ClientWorld worldIn, RayTraceResult result) {
        ...
        if (result.getType() == RayTraceResult.Type.BLOCK && TREE_BLOCKS.contains((blockstate = worldIn.getBlockState(((BlockRayTraceResult)result).getPos())).getBlock())) {
    ...
    public static boolean hasPunchedTreesPreviously(ClientPlayerEntity p_194070_0_) {
        for (Block block : TREE_BLOCKS) {
            if (p_194070_0_.getStats().getValue(Stats.BLOCK_MINED.get(block)) <= 0) continue;
            return true;
        }
        return false;
    }
