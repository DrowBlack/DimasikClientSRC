...
implements ISuggestionProvider {
    ...
    public CommandSource(ICommandSource sourceIn, Vector3d posIn, Vector2f rotationIn, ServerWorld worldIn, int permissionLevelIn, String nameIn, ITextComponent displayNameIn, MinecraftServer serverIn, @Nullable Entity entityIn) {
        this(sourceIn, posIn, rotationIn, worldIn, permissionLevelIn, nameIn, displayNameIn, serverIn, entityIn, false, (p_197032_0_, p_197032_1_, p_197032_2_) -> {}, EntityAnchorArgument.Type.FEET);
    }
    ...
    public CommandSource withResultConsumer(ResultConsumer<CommandSource> resultConsumerIn, BinaryOperator<ResultConsumer<CommandSource>> resultConsumerSelector) {
        ResultConsumer resultconsumer = (ResultConsumer)resultConsumerSelector.apply(this.resultConsumer, resultConsumerIn);
    ...
    public CommandSource withRotation(Vector3d lookPos) throws CommandSyntaxException {
        ...
        float f = MathHelper.wrapDegrees((float)(-(MathHelper.atan2(d1, d3) * 57.2957763671875)));
        float f1 = MathHelper.wrapDegrees((float)(MathHelper.atan2(d2, d0) * 57.2957763671875) - 90.0f);
    ...
    private void logFeedback(ITextComponent message) {
        IFormattableTextComponent itextcomponent = new TranslationTextComponent("chat.type.admin", this.getDisplayName(), message).mergeStyle(TextFormatting.GRAY, TextFormatting.ITALIC);
