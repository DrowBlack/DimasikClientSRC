...
implements ArgumentType<ResourceLocation> {
    private static final Collection<String> EXAMPLES = Stream.of(World.OVERWORLD, World.THE_NETHER).map(worldKey -> worldKey.getLocation().toString()).collect(Collectors.toList());
    private static final DynamicCommandExceptionType INVALID_DIMENSION_EXCEPTION = new DynamicCommandExceptionType(worldKey -> new TranslationTextComponent("argument.dimension.invalid", worldKey));
