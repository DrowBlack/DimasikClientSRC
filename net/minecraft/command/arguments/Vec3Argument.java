...
implements ArgumentType<ILocationArgument> {
    ...
    public ILocationArgument parse(StringReader p_parse_1_) throws CommandSyntaxException {
        return p_parse_1_.canRead() && p_parse_1_.peek() == '^' ? LocalLocationArgument.parse(p_parse_1_) : LocationInput.parseDouble(p_parse_1_, this.centerIntegers);
    }
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> p_listSuggestions_1_, SuggestionsBuilder p_listSuggestions_2_) {
        ...
        Collection<ISuggestionProvider.Coordinates> collection = !s.isEmpty() && s.charAt(0) == '^' ? Collections.singleton(ISuggestionProvider.Coordinates.DEFAULT_LOCAL) : ((ISuggestionProvider)p_listSuggestions_1_.getSource()).func_217293_r();
