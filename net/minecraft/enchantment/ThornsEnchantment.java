...
extends Enchantment {
    ...
    public void onUserHurt(LivingEntity user, Entity attacker, int level) {
        ...
        if (ThornsEnchantment.shouldHit(level, random)) {
            if (attacker != null) {
                attacker.attackEntityFrom(DamageSource.causeThornsDamage(user), ThornsEnchantment.getDamage(level, random));
            }
            if (entry != null) {
                entry.getValue().damageItem(2, user, livingEntity -> livingEntity.sendBreakAnimation((EquipmentSlotType)((Object)((Object)entry.getKey()))));
            }
        }
    }
