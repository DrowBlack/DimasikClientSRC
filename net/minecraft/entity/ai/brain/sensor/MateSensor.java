...
extends Sensor<AgeableEntity> {
    ...
    protected void update(ServerWorld worldIn, AgeableEntity entityIn) {
        entityIn.getBrain().getMemory(MemoryModuleType.VISIBLE_MOBS).ifPresent(entities -> this.addMemory(entityIn, (List<LivingEntity>)entities));
    }

    private void addMemory(AgeableEntity entity, List<LivingEntity> entities) {
        Optional<AgeableEntity> optional = entities.stream().filter(livingEntity -> livingEntity.getType() == entity.getType()).map(livingEntity -> (AgeableEntity)livingEntity).filter(ageable -> !ageable.isChild()).findFirst();
