...
extends Task<E> {
    ...
    protected boolean shouldExecute(ServerWorld worldIn, E owner) {
        LivingEntity livingentity = ShootTargetTask.func_233887_a_(owner);
        return ((LivingEntity)owner).canEquip(Items.CROSSBOW) && BrainUtil.isMobVisible(owner, livingentity) && BrainUtil.canFireAtTarget(owner, livingentity, 0);
    }

    @Override
    protected boolean shouldContinueExecuting(ServerWorld worldIn, E entityIn, long gameTimeIn) {
        return ((LivingEntity)entityIn).getBrain().hasMemory(MemoryModuleType.ATTACK_TARGET) && this.shouldExecute(worldIn, entityIn);
    }

    @Override
    protected void updateTask(ServerWorld worldIn, E owner, long gameTime) {
        LivingEntity livingentity = ShootTargetTask.func_233887_a_(owner);
        this.func_233889_b_((MobEntity)owner, livingentity);
        this.func_233888_a_(owner, livingentity);
    }

    @Override
    protected void resetTask(ServerWorld worldIn, E entityIn, long gameTimeIn) {
        if (((LivingEntity)entityIn).isHandActive()) {
            ((LivingEntity)entityIn).resetActiveHand();
        }
        if (((LivingEntity)entityIn).canEquip(Items.CROSSBOW)) {
            ((ICrossbowUser)entityIn).setCharging(false);
            CrossbowItem.setCharged(((LivingEntity)entityIn).getActiveItemStack(), false);
        }
    }

    private void func_233888_a_(E p_233888_1_, LivingEntity p_233888_2_) {
        if (this.field_233886_c_ == Status.UNCHARGED) {
            ((LivingEntity)p_233888_1_).setActiveHand(ProjectileHelper.getHandWith(p_233888_1_, Items.CROSSBOW));
            this.field_233886_c_ = Status.CHARGING;
            ((ICrossbowUser)p_233888_1_).setCharging(true);
        } else if (this.field_233886_c_ == Status.CHARGING) {
            ItemStack itemstack;
            int i;
            if (!((LivingEntity)p_233888_1_).isHandActive()) {
                this.field_233886_c_ = Status.UNCHARGED;
            }
            if ((i = ((LivingEntity)p_233888_1_).getItemInUseMaxCount()) >= CrossbowItem.getChargeTime(itemstack = ((LivingEntity)p_233888_1_).getActiveItemStack())) {
                ((LivingEntity)p_233888_1_).stopActiveHand();
                this.field_233886_c_ = Status.CHARGED;
                this.field_233885_b_ = 20 + ((LivingEntity)p_233888_1_).getRNG().nextInt(20);
                ((ICrossbowUser)p_233888_1_).setCharging(false);
            }
            ...
            ((IRangedAttackMob)p_233888_1_).attackEntityWithRangedAttack(p_233888_2_, 1.0f);
            ItemStack itemstack1 = ((LivingEntity)p_233888_1_).getHeldItem(ProjectileHelper.getHandWith(p_233888_1_, Items.CROSSBOW));
