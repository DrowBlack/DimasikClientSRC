...
implements IAngerable {
    ...
    public void readAdditional(CompoundNBT compound) {
        ...
        if (compound.contains("carriedBlockState", 10) && (blockstate = NBTUtil.readBlockState(compound.getCompound("carriedBlockState"))).isAir()) {
            blockstate = null;
        }
    ...
    private boolean shouldAttackPlayer(PlayerEntity player) {
        ...
        double d1 = vector3d.dotProduct(vector3d1 = vector3d1.normalize());
    ...
    extends Goal {
        ...
        public void tick() {
            ...
            int i = MathHelper.floor(this.enderman.getPosX() - 1.0 + random.nextDouble() * 2.0);
            int j = MathHelper.floor(this.enderman.getPosY() + random.nextDouble() * 2.0);
            int k = MathHelper.floor(this.enderman.getPosZ() - 1.0 + random.nextDouble() * 2.0);
            BlockPos blockpos = new BlockPos(i, j, k);
            BlockState blockstate = world.getBlockState(blockpos);
            BlockPos blockpos1 = blockpos.down();
            BlockState blockstate1 = world.getBlockState(blockpos1);
            BlockState blockstate2 = this.enderman.getHeldBlockState();
            if (blockstate2 != null && this.func_220836_a(world, blockpos, blockstate2 = Block.getValidBlockForPosition(blockstate2, this.enderman.world, blockpos), blockstate, blockstate1, blockpos1)) {
    ...
    extends Goal {
        ...
        public void tick() {
            Random random = this.enderman.getRNG();
            World world = this.enderman.world;
            int i = MathHelper.floor(this.enderman.getPosX() - 2.0 + random.nextDouble() * 4.0);
            int j = MathHelper.floor(this.enderman.getPosY() + random.nextDouble() * 3.0);
            int k = MathHelper.floor(this.enderman.getPosZ() - 2.0 + random.nextDouble() * 4.0);
            BlockPos blockpos = new BlockPos(i, j, k);
            BlockState blockstate = world.getBlockState(blockpos);
            Block block = blockstate.getBlock();
            Vector3d vector3d = new Vector3d((double)MathHelper.floor(this.enderman.getPosX()) + 0.5, (double)j + 0.5, (double)MathHelper.floor(this.enderman.getPosZ()) + 0.5);
    ...
    extends NearestAttackableTargetGoal<PlayerEntity> {
        ...
        public FindPlayerGoal(EndermanEntity p_i241912_1_, @Nullable Predicate<LivingEntity> p_i241912_2_) {
            ...
            this.field_220791_m = new EntityPredicate().setDistance(this.getTargetDistance()).setCustomPredicate(p_220790_1_ -> p_i241912_1_.shouldAttackPlayer((PlayerEntity)p_220790_1_));
        }
