...
extends Task<E> {
    ...
    protected boolean shouldExecute(ServerWorld worldIn, E owner) {
        ItemEntity itementity = ((PiglinEntity)owner).getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_WANTED_ITEM).get();
        return PiglinTasks.func_234480_a_(itementity.getItem().getItem());
    }

    @Override
    protected void startExecuting(ServerWorld worldIn, E entityIn, long gameTimeIn) {
        ((PiglinEntity)entityIn).getBrain().replaceMemory(MemoryModuleType.ADMIRING_ITEM, true, this.field_234540_b_);
    }
}
