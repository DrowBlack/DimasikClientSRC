...
INameable {
    ...
    public int findSlotMatchingUnusedItem(ItemStack p_194014_1_) {
        for (int i = 0; i < this.mainInventory.size(); ++i) {
            ...
            if (this.mainInventory.get(i).isEmpty() || !this.stackEqualExact(p_194014_1_, this.mainInventory.get(i)) || this.mainInventory.get(i).isDamaged() || itemstack.isEnchanted() || itemstack.hasDisplayName()) continue;
            return i;
        }
        return -1;
    }
    ...
    public int getBestHotbarSlot() {
        ...
        for (int k = 0; k < 9; ++k) {
            ...
            if (this.mainInventory.get(l).isEnchanted()) continue;
            return l;
        }
    ...
    public void changeCurrentItem(double direction) {
        ...
        this.currentItem = (int)((double)this.currentItem - direction);
    ...
    public int func_234564_a_(Predicate<ItemStack> p_234564_1_, int p_234564_2_, IInventory p_234564_3_) {
        ...
        i += ItemStackHelper.func_233534_a_(this, p_234564_1_, p_234564_2_ - i, flag);
        i += ItemStackHelper.func_233534_a_(p_234564_3_, p_234564_1_, p_234564_2_ - i, flag);
        i += ItemStackHelper.func_233535_a_(this.itemStack, p_234564_1_, p_234564_2_ - i, flag);
    ...
    public void placeItemBackInInventory(World worldIn, ItemStack stack) {
        if (!worldIn.isRemote) {
            while (!stack.isEmpty()) {
                ...
                if (!this.add(i, stack.split(j))) continue;
    ...
    public void func_234563_a_(DamageSource p_234563_1_, float p_234563_2_) {
        if (!(p_234563_2_ <= 0.0f)) {
            ...
            for (int i = 0; i < this.armorInventory.size(); ++i) {
                ...
                if (p_234563_1_.isFireDamage() && itemstack.getItem().isImmuneToFire() || !(itemstack.getItem() instanceof ArmorItem)) continue;
                int j = i;
                itemstack.damageItem((int)p_234563_2_, this.player, p_214023_1_ -> p_214023_1_.sendBreakAnimation(EquipmentSlotType.fromSlotTypeAndIndex(EquipmentSlotType.Group.ARMOR, j)));
            }
        }
    }
    ...
    public boolean hasTag(ITag<Item> itemTag) {
        for (List list : this.allInventories) {
            for (ItemStack itemstack : list) {
                if (itemstack.isEmpty() || !itemTag.contains(itemstack.getItem())) continue;
                return true;
            }
        }
        return false;
