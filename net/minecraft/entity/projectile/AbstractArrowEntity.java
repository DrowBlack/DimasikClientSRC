...
extends ProjectileEntity {
    ...
    public boolean isInRangeToRenderDist(double distance) {
        ...
        return distance < (d0 = d0 * 64.0 * AbstractArrowEntity.getRenderDistanceWeight()) * d0;
    }
    ...
    public void tick() {
        ...
        if (this.prevRotationPitch == 0.0f && this.prevRotationYaw == 0.0f) {
            float f = MathHelper.sqrt(AbstractArrowEntity.horizontalMag(vector3d));
            this.rotationYaw = (float)(MathHelper.atan2(vector3d.x, vector3d.z) * 57.2957763671875);
            this.rotationPitch = (float)(MathHelper.atan2(vector3d.y, f) * 57.2957763671875);
            this.prevRotationYaw = this.rotationYaw;
            this.prevRotationPitch = this.rotationPitch;
        }
        if (!((blockstate = this.world.getBlockState(blockpos = this.getPosition())).isAir() || flag || (voxelshape = blockstate.getCollisionShape(this.world, blockpos)).isEmpty())) {
        ...
        if (this.inGround && !flag) {
            ...
            RayTraceResult raytraceresult = this.world.rayTraceBlocks(new RayTraceContext(vector3d2, vector3d3 = vector3d2.add(vector3d), RayTraceContext.BlockMode.COLLIDER, RayTraceContext.FluidMode.NONE, this));
            ...
            float f1 = MathHelper.sqrt(AbstractArrowEntity.horizontalMag(vector3d));
            this.rotationYaw = flag ? (float)(MathHelper.atan2(-d3, -d0) * 57.2957763671875) : (float)(MathHelper.atan2(d3, d0) * 57.2957763671875);
            this.rotationPitch = (float)(MathHelper.atan2(d4, f1) * 57.2957763671875);
    ...
    protected void onEntityHit(EntityRayTraceResult p_213868_1_) {
        ...
        if (entity.attackEntityFrom(damagesource, i)) {
            ...
            if (entity instanceof LivingEntity) {
                ...
                if (this.knockbackStrength > 0 && (vector3d = this.getMotion().mul(1.0, 0.0, 1.0).normalize().scale((double)this.knockbackStrength * 0.6)).lengthSquared() > 0.0) {
                    livingentity.addVelocity(vector3d.x, 0.1, vector3d.z);
                }
    ...
    protected EntityRayTraceResult rayTraceEntities(Vector3d startVec, Vector3d endVec) {
        return ProjectileHelper.rayTraceEntities(this.world, this, startVec, endVec, this.getBoundingBox().expand(this.getMotion()).grow(1.0), this::func_230298_a_);
    }
    ...
    public void onCollideWithPlayer(PlayerEntity entityIn) {
        if (!this.world.isRemote && (this.inGround || this.getNoClip()) && this.arrowShake <= 0) {
            boolean flag;
            boolean bl = flag = this.pickupStatus == PickupStatus.ALLOWED || this.pickupStatus == PickupStatus.CREATIVE_ONLY && entityIn.abilities.isCreativeMode || this.getNoClip() && this.func_234616_v_().getUniqueID() == entityIn.getUniqueID();
    ...
    public void setEnchantmentEffectsFromEntity(LivingEntity p_190547_1_, float p_190547_2_) {
        ...
        this.setDamage((double)(p_190547_2_ * 2.0f) + this.rand.nextGaussian() * 0.25 + (double)((float)this.world.getDifficulty().getId() * 0.11f));
        if (i > 0) {
