...
extends AbstractArrowEntity {
    ...
    public void setPotionEffect(ItemStack stack) {
        if (stack.getItem() == Items.TIPPED_ARROW) {
            ...
            List<EffectInstance> collection = PotionUtils.getFullEffectsFromItem(stack);
            if (!collection.isEmpty()) {
                for (EffectInstance effectinstance : collection) {
                    this.customPotionEffects.add(new EffectInstance(effectinstance));
                }
            }
            if ((i = ArrowEntity.getCustomColor(stack)) == -1) {
                this.refreshColor();
            } else {
    ...
    private void spawnPotionParticles(int particleCount) {
        ...
        if (i != -1 && particleCount > 0) {
            ...
            for (int j = 0; j < particleCount; ++j) {
                this.world.addParticle(ParticleTypes.ENTITY_EFFECT, this.getPosXRandom(0.5), this.getPosYRandom(), this.getPosZRandom(0.5), d0, d1, d2);
            }
        }
    }
