...
extends Item {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
        ...
        BlockRayTraceResult raytraceresult = BucketItem.rayTrace(worldIn, playerIn, this.containedBlock == Fluids.EMPTY ? RayTraceContext.FluidMode.SOURCE_ONLY : RayTraceContext.FluidMode.NONE);
        ...
        if (worldIn.isBlockModifiable(playerIn, blockpos) && playerIn.canPlayerEdit(blockpos1, direction, itemstack)) {
            ...
            if (this.containedBlock == Fluids.EMPTY) {
                ...
                if (blockstate1.getBlock() instanceof IBucketPickupHandler && (fluid = ((IBucketPickupHandler)((Object)blockstate1.getBlock())).pickupFluid(worldIn, blockpos, blockstate1)) != Fluids.EMPTY) {
            ...
            BlockPos blockPos = blockpos2 = blockstate.getBlock() instanceof ILiquidContainer && this.containedBlock == Fluids.WATER ? blockpos : blockpos1;
    ...
    public boolean tryPlaceContainedLiquid(@Nullable PlayerEntity player, World worldIn, BlockPos posIn, @Nullable BlockRayTraceResult rayTrace) {
        ...
        boolean bl = flag1 = blockstate.isAir() || flag || block instanceof ILiquidContainer && ((ILiquidContainer)((Object)block)).canContainFluid(worldIn, posIn, blockstate, this.containedBlock);
        if (!flag1) {
        ...
        if (worldIn.getDimensionType().isUltrawarm() && this.containedBlock.isIn(FluidTags.WATER)) {
            ...
            for (int l = 0; l < 8; ++l) {
                worldIn.addParticle(ParticleTypes.LARGE_SMOKE, (double)i + Math.random(), (double)j + Math.random(), (double)k + Math.random(), 0.0, 0.0, 0.0);
            }
            return true;
        }
        if (block instanceof ILiquidContainer && this.containedBlock == Fluids.WATER) {
            ((ILiquidContainer)((Object)block)).receiveFluid(worldIn, posIn, blockstate, ((FlowingFluid)this.containedBlock).getStillFluidState(false));
