...
extends AbstractMapItem {
    ...
    public static ItemStack setupNewMap(World worldIn, int worldX, int worldZ, byte scale, boolean trackingPosition, boolean unlimitedTracking) {
        ...
        FilledMapItem.createMapData(itemstack, worldIn, worldX, worldZ, scale, trackingPosition, unlimitedTracking, worldIn.getDimensionKey());
        return itemstack;
    }

    @Nullable
    public static MapData getData(ItemStack stack, World worldIn) {
        return worldIn.getMapData(FilledMapItem.getMapName(FilledMapItem.getMapId(stack)));
    }
    ...
    public static MapData getMapData(ItemStack stack, World worldIn) {
        ...
        if (mapdata == null && worldIn instanceof ServerWorld) {
            mapdata = FilledMapItem.createMapData(stack, worldIn, worldIn.getWorldInfo().getSpawnX(), worldIn.getWorldInfo().getSpawnZ(), 3, false, false, worldIn.getDimensionKey());
        }
        return mapdata;
    }
    ...
    private static MapData createMapData(ItemStack stack, World worldIn, int x, int z, int scale, boolean trackingPosition, boolean unlimitedTracking, RegistryKey<World> dimensionTypeIn) {
        ...
        MapData mapdata = new MapData(FilledMapItem.getMapName(i));
        ...
        if (worldIn.getDimensionKey() == data.dimension && viewer instanceof PlayerEntity) {
            ...
            for (int k1 = l - j1 + 1; k1 < l + j1; ++k1) {
                ...
                for (int l1 = i1 - j1 - 1; l1 < i1 + j1; ++l1) {
                        ...
                        for (int i4 = 0; i4 < i; ++i4) {
                            for (int j4 = 0; j4 < i; ++j4) {
                                ...
                                if (k4 <= 1) {
                                    ...
                                    } while ((blockstate = chunk.getBlockState(blockpos$mutable1)).getMaterialColor(worldIn, blockpos$mutable1) == MaterialColor.AIR && k4 > 0);
                    ...
                    d0 = d1;
                    if (l1 < 0 || i2 * i2 + j2 * j2 >= j1 * j1 || flag1 && (k1 + l1 & 1) == 0 || (b0 = data.colors[k1 + l1 * 128]) == (b1 = (byte)(materialcolor.colorIndex * 4 + i5))) continue;
    ...
    public static void func_226642_a_(ServerWorld p_226642_0_, ItemStack p_226642_1_) {
        MapData mapdata = FilledMapItem.getMapData(p_226642_1_, p_226642_0_);
        if (mapdata != null && p_226642_0_.getDimensionKey() == mapdata.dimension) {
    ...
    public void inventoryTick(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        MapData mapdata;
        if (!worldIn.isRemote && (mapdata = FilledMapItem.getMapData(stack, worldIn)) != null) {
    ...
    public IPacket<?> getUpdatePacket(ItemStack stack, World worldIn, PlayerEntity player) {
        return FilledMapItem.getMapData(stack, worldIn).getMapPacket(stack, worldIn, player);
    }
    ...
    public void onCreated(ItemStack stack, World worldIn, PlayerEntity playerIn) {
        ...
        if (compoundnbt != null && compoundnbt.contains("map_scale_direction", 99)) {
            FilledMapItem.scaleMap(stack, worldIn, compoundnbt.getInt("map_scale_direction"));
            ...
            FilledMapItem.func_219992_b(worldIn, stack);
            compoundnbt.remove("map_to_lock");
        }
    }

    protected static void scaleMap(ItemStack p_185063_0_, World p_185063_1_, int p_185063_2_) {
        MapData mapdata = FilledMapItem.getMapData(p_185063_0_, p_185063_1_);
        if (mapdata != null) {
            FilledMapItem.createMapData(p_185063_0_, p_185063_1_, mapdata.xCenter, mapdata.zCenter, MathHelper.clamp(mapdata.scale + p_185063_2_, 0, 4), mapdata.trackingPosition, mapdata.unlimitedTracking, mapdata.dimension);
        }
    }
    ...
    public static void func_219992_b(World worldIn, ItemStack stack) {
        ...
        if (mapdata != null) {
            MapData mapdata1 = FilledMapItem.createMapData(stack, worldIn, 0, 0, mapdata.scale, mapdata.trackingPosition, mapdata.unlimitedTracking, mapdata.dimension);
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        MapData mapdata;
        MapData mapData = mapdata = worldIn == null ? null : FilledMapItem.getMapData(stack, worldIn);
        if (mapdata != null && mapdata.locked) {
            tooltip.add(new TranslationTextComponent("filled_map.locked", FilledMapItem.getMapId(stack)).mergeStyle(TextFormatting.GRAY));
        }
        if (flagIn.isAdvanced()) {
            if (mapdata != null) {
                tooltip.add(new TranslationTextComponent("filled_map.id", FilledMapItem.getMapId(stack)).mergeStyle(TextFormatting.GRAY));
    ...
    public ActionResultType onItemUse(ItemUseContext context) {
        ...
        if (blockstate.isIn(BlockTags.BANNERS)) {
            if (!context.getWorld().isRemote) {
                MapData mapdata = FilledMapItem.getMapData(context.getItem(), context.getWorld());
