...
extends Item {
    ...
    public ActionResultType onItemUse(ItemUseContext context) {
        ...
        if (!world.isRemote) {
            ...
            FireworkRocketEntity fireworkrocketentity = new FireworkRocketEntity(world, context.getPlayer(), vector3d.x + (double)direction.getXOffset() * 0.15, vector3d.y + (double)direction.getYOffset() * 0.15, vector3d.z + (double)direction.getZOffset() * 0.15, itemstack);
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        ...
        if (compoundnbt != null) {
            ...
            if (compoundnbt.contains("Flight", 99)) {
                tooltip.add(new TranslationTextComponent("item.minecraft.firework_rocket.flight").appendString(" ").appendString(String.valueOf(compoundnbt.getByte("Flight"))).mergeStyle(TextFormatting.GRAY));
            }
            if (!(listnbt = compoundnbt.getList("Explosions", 10)).isEmpty()) {
                for (int i = 0; i < listnbt.size(); ++i) {
                    ...
                    for (int j = 1; j < list.size(); ++j) {
                        list.set(j, new StringTextComponent("  ").append((ITextComponent)list.get(j)).mergeStyle(TextFormatting.GRAY));
                    }
                    tooltip.addAll(list);
                }
            }
    ...
    public static enum Shape {
        ...
        static {
            VALUES = (Shape[])Arrays.stream(Shape.values()).sorted(Comparator.comparingInt(p_199796_0_ -> p_199796_0_.index)).toArray(Shape[]::new);
        }
    }
}
