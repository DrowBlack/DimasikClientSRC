...
extends BucketItem {
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        ...
        if (this.fishType == EntityType.TROPICAL_FISH && (compoundnbt = stack.getTag()) != null && compoundnbt.contains("BucketVariantTag", 3)) {
            int i = compoundnbt.getInt("BucketVariantTag");
            TextFormatting[] atextformatting = new TextFormatting[]{TextFormatting.ITALIC, TextFormatting.GRAY};
            String s = "color.minecraft." + String.valueOf(TropicalFishEntity.func_212326_d(i));
            String s1 = "color.minecraft." + String.valueOf(TropicalFishEntity.func_212323_p(i));
            for (int j = 0; j < TropicalFishEntity.SPECIAL_VARIANTS.length; ++j) {
                if (i != TropicalFishEntity.SPECIAL_VARIANTS[j]) continue;
