...
import dimasik.itemics.api.utils.accessor.IItemStack;
...
implements IItemStack {
    public static final Codec<ItemStack> CODEC = RecordCodecBuilder.create(p_234698_0_ -> p_234698_0_.group(((MapCodec)Registry.ITEM.fieldOf("id")).forGetter(p_234706_0_ -> p_234706_0_.item), ((MapCodec)Codec.INT.fieldOf("Count")).forGetter(p_234705_0_ -> p_234705_0_.count), CompoundNBT.CODEC.optionalFieldOf("tag").forGetter(p_234704_0_ -> Optional.ofNullable(p_234704_0_.tag))).apply((Applicative<ItemStack, ?>)p_234698_0_, ItemStack::new));
    private static final Logger LOGGER = LogManager.getLogger();
    public static final ItemStack EMPTY = new ItemStack((IItemProvider)null);
    public static final DecimalFormat DECIMALFORMAT = Util.make(new DecimalFormat("#.##"), p_234699_0_ -> p_234699_0_.setDecimalFormatSymbols(DecimalFormatSymbols.getInstance(Locale.ROOT)));
    ...
    private void recalculateHash() {
        this.itemicsHash = this.item == null ? -1 : this.item.hashCode() + this.getDamage();
    }
    ...
    public static ItemStack read(CompoundNBT compound) {
        try {
            ...
            LOGGER.debug("Tried to load invalid item: {}", (Object)compound, (Object)runtimeexception);
            return EMPTY;
        }
    }
    ...
    public ItemStack onItemUseFinish(World worldIn, LivingEntity entityLiving) {
        ...
        ItemsCooldown.ItemEnum itemEnum = ItemsCooldown.ItemEnum.getItemEnum(this.getItem());
    ...
    public boolean attemptDamageItem(int amount, Random rand, @Nullable ServerPlayerEntity damager) {
        ...
        if (amount > 0) {
            ...
            for (int k = 0; i > 0 && k < amount; ++k) {
                if (!UnbreakingEnchantment.negateDamage(this, i, rand)) continue;
                ++j;
            }
            if ((amount -= j) <= 0) {
                return false;
    ...
    public <T extends LivingEntity> void damageItem(int amount, T entityIn, Consumer<T> onBroken) {
        if (!(entityIn.world.isRemote || entityIn instanceof PlayerEntity && ((PlayerEntity)entityIn).abilities.isCreativeMode || !this.isDamageable() || !this.attemptDamageItem(amount, entityIn.getRNG(), entityIn instanceof ServerPlayerEntity ? (ServerPlayerEntity)entityIn : null))) {
    ...
    public String toString() {
        return this.count + " " + String.valueOf(this.getItem());
    }
    ...
    public List<ITextComponent> getTooltip(@Nullable PlayerEntity playerIn, ITooltipFlag advanced) {
        ...
        if (this.hasTag()) {
            if (ItemStack.func_242394_a(i, TooltipDisplayFlags.ENCHANTMENTS)) {
                ItemStack.addEnchantmentTooltips(list, this.getEnchantmentTagList());
            }
            if (this.tag.contains("display", 10)) {
                CompoundNBT compoundnbt = this.tag.getCompound("display");
                if (ItemStack.func_242394_a(i, TooltipDisplayFlags.DYE) && compoundnbt.contains("color", 99)) {
                ...
                for (Map.Entry<Attribute, AttributeModifier> entry : multimap.entries()) {
                    ...
                    if (playerIn != null) {
                        if (attributemodifier.getID() == Item.ATTACK_DAMAGE_MODIFIER) {
                            d0 += playerIn.getBaseAttributeValue(Attributes.ATTACK_DAMAGE);
                    ...
                    double d1 = attributemodifier.getOperation() != AttributeModifier.Operation.MULTIPLY_BASE && attributemodifier.getOperation() != AttributeModifier.Operation.MULTIPLY_TOTAL ? (entry.getKey().equals(Attributes.KNOCKBACK_RESISTANCE) ? d0 * 10.0 : d0) : d0 * 100.0;
                    if (flag) {
                    ...
                    if (!(d0 < 0.0)) continue;
                    list.add(new TranslationTextComponent("attribute.modifier.take." + attributemodifier.getOperation().getId(), DECIMALFORMAT.format(d1 *= -1.0), new TranslationTextComponent(entry.getKey().getAttributeName())).mergeStyle(TextFormatting.RED));
                }
            }
        }
        if (this.hasTag()) {
            ListNBT listnbt2;
            ListNBT listnbt1;
            if (ItemStack.func_242394_a(i, TooltipDisplayFlags.UNBREAKABLE) && this.tag.getBoolean("Unbreakable")) {
                list.add(new TranslationTextComponent("item.unbreakable").mergeStyle(TextFormatting.BLUE));
            }
            if (ItemStack.func_242394_a(i, TooltipDisplayFlags.CAN_DESTROY) && this.tag.contains("CanDestroy", 9) && !(listnbt1 = this.tag.getList("CanDestroy", 8)).isEmpty()) {
                list.add(StringTextComponent.EMPTY);
                list.add(new TranslationTextComponent("item.canBreak").mergeStyle(TextFormatting.GRAY));
                for (int k = 0; k < listnbt1.size(); ++k) {
                    list.addAll(ItemStack.getPlacementTooltip(listnbt1.getString(k)));
                }
            }
            if (ItemStack.func_242394_a(i, TooltipDisplayFlags.CAN_PLACE) && this.tag.contains("CanPlaceOn", 9) && !(listnbt2 = this.tag.getList("CanPlaceOn", 8)).isEmpty()) {
                list.add(StringTextComponent.EMPTY);
                list.add(new TranslationTextComponent("item.canPlace").mergeStyle(TextFormatting.GRAY));
                for (int l = 0; l < listnbt2.size(); ++l) {
                    list.addAll(ItemStack.getPlacementTooltip(listnbt2.getString(l)));
                }
            }
        }
        if (advanced.isAdvanced()) {
    ...
    public static void addEnchantmentTooltips(List<ITextComponent> p_222120_0_, ListNBT p_222120_1_) {
        for (int i = 0; i < p_222120_1_.size(); ++i) {
            ...
            Registry.ENCHANTMENT.getOptional(ResourceLocation.tryCreate(compoundnbt.getString("id"))).ifPresent(p_222123_2_ -> p_222120_0_.add(p_222123_2_.getDisplayName(compoundnbt.getInt("lvl"))));
        }
    }
    ...
    private static Collection<ITextComponent> getPlacementTooltip(String stateString) {
        try {
            ...
            if (flag || flag1) {
                ...
                if (itag != null && !(collection = itag.getAllElements()).isEmpty()) {
                    return collection.stream().map(Block::getTranslatedName).map(p_222119_0_ -> p_222119_0_.mergeStyle(TextFormatting.DARK_GRAY)).collect(Collectors.toList());
                }
            }
        }
    ...
    public ITextComponent getTextComponent() {
        ...
        if (!this.isEmpty) {
            iformattabletextcomponent1.mergeStyle(this.getRarity().color).modifyStyle(p_234702_1_ -> p_234702_1_.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_ITEM, new HoverEvent.ItemHover(this))));
        }
    ...
    public boolean canDestroy(ITagCollectionSupplier p_206848_1_, CachedBlockInfo p_206848_2_) {
        if (ItemStack.isStateAndTileEntityEqual(p_206848_2_, this.canDestroyCacheBlock)) {
    ...
    public boolean canPlaceOn(ITagCollectionSupplier p_206847_1_, CachedBlockInfo p_206847_2_) {
        if (ItemStack.isStateAndTileEntityEqual(p_206847_2_, this.canPlaceOnCacheBlock)) {
