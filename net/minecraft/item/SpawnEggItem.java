...
extends Item {
    ...
    public ActionResultType onItemUse(ItemUseContext context) {
        ...
        if (blockstate.isIn(Blocks.SPAWNER) && (tileentity = world.getTileEntity(blockpos)) instanceof MobSpawnerTileEntity) {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
        ...
        BlockRayTraceResult raytraceresult = SpawnEggItem.rayTrace(worldIn, playerIn, RayTraceContext.FluidMode.SOURCE_ONLY);
    ...
    public EntityType<?> getType(@Nullable CompoundNBT nbt) {
        ...
        if (nbt != null && nbt.contains("EntityTag", 10) && (compoundnbt = nbt.getCompound("EntityTag")).contains("id", 8)) {
    ...
    public Optional<MobEntity> getChildToSpawn(PlayerEntity player, MobEntity mob, EntityType<? extends MobEntity> entityType, ServerWorld world, Vector3d pos, ItemStack stack) {
        ...
        MobEntity mobentity = mob instanceof AgeableEntity ? ((AgeableEntity)mob).func_241840_a(world, (AgeableEntity)mob) : entityType.create(world);
        if (mobentity == null) {
            return Optional.empty();
        }
