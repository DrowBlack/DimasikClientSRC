...
public class LootTable {
    ...
    public static Consumer<ItemStack> capStackSizes(Consumer<ItemStack> stackConsumer) {
        return stack -> {
            if (stack.getCount() < stack.getMaxStackSize()) {
                stackConsumer.accept((ItemStack)stack);
            } else {
                ItemStack itemstack;
    ...
    public void generate(LootContext contextData, Consumer<ItemStack> stacksOut) {
        this.recursiveGenerate(contextData, LootTable.capStackSizes(stacksOut));
    }
    ...
    private void shuffleItems(List<ItemStack> stacks, int emptySlotsCount, Random rand) {
        ...
        while (emptySlotsCount - stacks.size() - list.size() > 0 && !list.isEmpty()) {
            ItemStack itemstack2 = (ItemStack)list.remove(MathHelper.nextInt(rand, 0, list.size() - 1));
    ...
    implements ILootFunctionConsumer<Builder> {
        ...
        public Builder addLootPool(LootPool.Builder lootPoolIn) {
            this.lootPools.add(lootPoolIn.build());
            return this;
        }

        public Builder setParameterSet(LootParameterSet parameterSet) {
            this.parameterSet = parameterSet;
            return this;
        }

        @Override
        public Builder acceptFunction(ILootFunction.IBuilder functionBuilder) {
    ...
    JsonSerializer<LootTable> {
        ...
        public JsonElement serialize(LootTable p_serialize_1_, Type p_serialize_2_, JsonSerializationContext p_serialize_3_) {
            ...
            if (p_serialize_1_.parameterSet != DEFAULT_PARAMETER_SET) {
                ...
                if (resourcelocation != null) {
                    ...
                    LOGGER.warn("Failed to find id for param set " + String.valueOf(p_serialize_1_.parameterSet));
                }
            }
            if (p_serialize_1_.pools.length > 0) {
                jsonobject.add("pools", p_serialize_3_.serialize(p_serialize_1_.pools));
            }
            if (!ArrayUtils.isEmpty(p_serialize_1_.functions)) {
