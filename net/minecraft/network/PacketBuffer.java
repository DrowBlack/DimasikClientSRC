...
implements IForgePacketBuffer {
    ...
    public <T> T func_240628_a_(Codec<T> p_240628_1_) throws IOException {
        ...
        DataResult dataresult = p_240628_1_.parse(NBTDynamicOps.INSTANCE, compoundnbt);
        if (dataresult.error().isPresent()) {
            throw new IOException("Failed to decode: " + dataresult.error().get().message() + " " + String.valueOf(compoundnbt));
        }
        return (T)dataresult.result().get();
    }

    public <T> void func_240629_a_(Codec<T> p_240629_1_, T p_240629_2_) throws IOException {
        DataResult<INBT> dataresult = p_240629_1_.encodeStart(NBTDynamicOps.INSTANCE, (INBT)p_240629_2_);
        if (dataresult.error().isPresent()) {
            throw new IOException("Failed to encode: " + dataresult.error().get().message() + " " + String.valueOf(p_240629_2_));
        }
    ...
    public PacketBuffer writeItemStack(ItemStack p_writeItemStack_1_, boolean p_writeItemStack_2_) {
        if (p_writeItemStack_1_.isEmpty()) {
            ...
            if (ReflectorForge.isDamageable(item, p_writeItemStack_1_) || item.shouldSyncTag()) {
                compoundnbt = p_writeItemStack_2_ && Reflector.IForgeItemStack_getShareTag.exists() ? (CompoundNBT)Reflector.call(p_writeItemStack_1_, Reflector.IForgeItemStack_getShareTag, new Object[0]) : p_writeItemStack_1_.getTag();
            }
