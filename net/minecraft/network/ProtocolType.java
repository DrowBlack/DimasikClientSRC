...
public enum ProtocolType {
    ...
    private final Map<PacketDirection, ? extends PacketList<?>> field_229711_h_;

    private static PacketRegistry func_229714_b_() {
        return new PacketRegistry();
    }

    private ProtocolType(int p_i226083_3_, PacketRegistry p_i226083_4_) {
    ...
    public Integer getPacketId(PacketDirection direction, IPacket<?> packetIn) {
        return this.field_229711_h_.get((Object)direction).func_229720_a_(packetIn.getClass());
    }

    @Nullable
    public IPacket<?> getPacket(PacketDirection direction, int packetId) {
        return this.field_229711_h_.get((Object)direction).func_229718_a_(packetId);
    }
    ...
    static {
        STATES_BY_ID = new ProtocolType[4];
        ...
        for (ProtocolType protocoltype : ProtocolType.values()) {
            ...
            ProtocolType.STATES_BY_ID[i - -1] = protocoltype;
            protocoltype.field_229711_h_.forEach((p_229713_1_, p_229713_2_) -> p_229713_2_.func_229717_a_().forEach(p_229712_1_ -> {
                if (STATES_BY_CLASS.containsKey(p_229712_1_) && STATES_BY_CLASS.get(p_229712_1_) != protocoltype) {
                    throw new IllegalStateException("Packet " + String.valueOf(p_229712_1_) + " is already assigned to protocol " + String.valueOf((Object)STATES_BY_CLASS.get(p_229712_1_)) + " - can't reassign to " + String.valueOf((Object)protocoltype));
                }
                STATES_BY_CLASS.put((Class<IPacket<?>>)p_229712_1_, protocoltype);
            }));
        }
    }

    static class PacketRegistry {
        private final Map<PacketDirection, PacketList<?>> field_229722_a_ = Maps.newEnumMap(PacketDirection.class);

        private PacketRegistry() {
        }

        public <T extends INetHandler> PacketRegistry func_229724_a_(PacketDirection p_229724_1_, PacketList<T> p_229724_2_) {
    ...
    static class PacketList<T extends INetHandler> {
        private final Object2IntMap<Class<? extends IPacket<T>>> field_229715_a_ = Util.make(new Object2IntOpenHashMap(), p_229719_0_ -> p_229719_0_.defaultReturnValue(-1));
        private final List<Supplier<? extends IPacket<T>>> field_229716_b_ = Lists.newArrayList();

        private PacketList() {
        }

        public <P extends IPacket<T>> PacketList<T> func_229721_a_(Class<P> p_229721_1_, Supplier<P> p_229721_2_) {
            int i = this.field_229716_b_.size();
            int j = this.field_229715_a_.put((Class<IPacket<T>>)p_229721_1_, i);
            if (j != -1) {
        ...
        public IPacket<?> func_229718_a_(int p_229718_1_) {
            Supplier<IPacket<T>> supplier = this.field_229716_b_.get(p_229718_1_);
