...
public class EntityDataManager {
    ...
    public <T> void register(DataParameter<T> key, T value) {
        ...
        if (DataSerializers.getSerializerId(key.getSerializer()) < 0) {
            throw new IllegalArgumentException("Unregistered serializer " + String.valueOf(key.getSerializer()) + " for " + i + "!");
        }
    ...
    public static void writeEntries(List<DataEntry<?>> entriesIn, PacketBuffer buf) throws IOException {
        if (entriesIn != null) {
            ...
            for (int i = 0; i < j; ++i) {
                EntityDataManager.writeEntry(buf, entriesIn.get(i));
            }
        }
        buf.writeByte(255);
    }
    ...
    private static <T> void writeEntry(PacketBuffer buf, DataEntry<T> entry) throws IOException {
        ...
        if (i < 0) {
            throw new EncoderException("Unknown serializer type " + String.valueOf(dataparameter.getSerializer()));
        }
    ...
    public static List<DataEntry<?>> readEntries(PacketBuffer buf) throws IOException {
        ...
        while ((i = buf.readUnsignedByte()) != 255) {
            ...
            if ((idataserializer = DataSerializers.getSerializer(j = buf.readVarInt())) == null) {
                throw new DecoderException("Unknown serializer type " + j);
            }
            list.add(EntityDataManager.makeDataEntry(buf, i, idataserializer));
        }
        return list;
    }

    private static <T> DataEntry<T> makeDataEntry(PacketBuffer bufferIn, int idIn, IDataSerializer<T> serializerIn) {
        return new DataEntry<T>(serializerIn.createKey(idIn), serializerIn.read(bufferIn));
    }
