...
public class Path {
    ...
    public Vector3d getVectorFromIndex(Entity entityIn, int index) {
        ...
        double d0 = (double)pathpoint.x + (double)((int)(entityIn.getWidth() + 1.0f)) * 0.5;
        double d1 = pathpoint.y;
        double d2 = (double)pathpoint.z + (double)((int)(entityIn.getWidth() + 1.0f)) * 0.5;
    ...
    public boolean isSamePath(@Nullable Path pathentityIn) {
        ...
        if (pathentityIn.points.size() != this.points.size()) {
            return false;
        }
        for (int i = 0; i < this.points.size(); ++i) {
            PathPoint pathpoint = this.points.get(i);
            PathPoint pathpoint1 = pathentityIn.points.get(i);
            if (pathpoint.x == pathpoint1.x && pathpoint.y == pathpoint1.y && pathpoint.z == pathpoint1.z) continue;
            return false;
        }
        return true;
    }
    ...
    public static Path read(PacketBuffer buf) {
        ...
        int i = buf.readInt();
        int j = buf.readInt();
        HashSet<FlaggedPathPoint> set = Sets.newHashSet();
        for (int k = 0; k < j; ++k) {
