...
public class PathFinder {
    ...
    public Path func_227478_a_(Region p_227478_1_, MobEntity p_227478_2_, Set<BlockPos> p_227478_3_, float p_227478_4_, int p_227478_5_, float p_227478_6_) {
        ...
        Map<FlaggedPathPoint, BlockPos> map = p_227478_3_.stream().collect(Collectors.toMap(p_224782_1_ -> this.nodeProcessor.func_224768_a(p_224782_1_.getX(), p_224782_1_.getY(), p_224782_1_.getZ()), Function.identity()));
    ...
    private Path func_227479_a_(PathPoint p_227479_1_, Map<FlaggedPathPoint, BlockPos> p_227479_2_, float p_227479_3_, int p_227479_4_, float p_227479_5_) {
        ...
        p_227479_1_.distanceToTarget = p_227479_1_.distanceToNext = this.func_224776_a(p_227479_1_, set);
        this.path.clearPath();
        this.path.addPoint(p_227479_1_);
        ImmutableSet set1 = ImmutableSet.of();
        int i = 0;
        HashSet<FlaggedPathPoint> set2 = Sets.newHashSetWithExpectedSize(set.size());
        int j = (int)((float)this.field_215751_d * p_227479_5_);
        while (!this.path.isPathEmpty() && ++i < j) {
            PathPoint pathpoint = this.path.dequeue();
            pathpoint.visited = true;
            for (FlaggedPathPoint flaggedpathpoint : set) {
                if (!(pathpoint.func_224757_c(flaggedpathpoint) <= (float)p_227479_4_)) continue;
                flaggedpathpoint.func_224764_e();
                set2.add(flaggedpathpoint);
            }
            if (!set2.isEmpty()) break;
            if (pathpoint.distanceTo(p_227479_1_) >= p_227479_3_) continue;
            ...
            for (int l = 0; l < k; ++l) {
                ...
                if (!(pathpoint1.field_222861_j < p_227479_3_) || pathpoint1.isAssigned() && !(f1 < pathpoint1.totalPathDistance)) continue;
        ...
        Optional<Path> optional = !set2.isEmpty() ? set2.stream().map(p_224778_2_ -> this.func_224780_a(p_224778_2_.func_224763_d(), (BlockPos)p_227479_2_.get(p_224778_2_), true)).min(Comparator.comparingInt(Path::getCurrentPathLength)) : set.stream().map(p_224777_2_ -> this.func_224780_a(p_224777_2_.func_224763_d(), (BlockPos)p_227479_2_.get(p_224777_2_), false)).min(Comparator.comparingDouble(Path::func_224769_l).thenComparingInt(Path::getCurrentPathLength));
    ...
    private Path func_224780_a(PathPoint p_224780_1_, BlockPos p_224780_2_, boolean p_224780_3_) {
        ArrayList<PathPoint> list = Lists.newArrayList();
