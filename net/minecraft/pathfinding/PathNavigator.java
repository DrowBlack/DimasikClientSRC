...
public abstract class PathNavigator {
    ...
    public PathNavigator(MobEntity entityIn, World worldIn) {
        ...
        this.world = worldIn;
        int i = MathHelper.floor(entityIn.getAttributeValue(Attributes.FOLLOW_RANGE) * 16.0);
    ...
    protected Path pathfind(Set<BlockPos> positions, int regionOffset, boolean offsetUpward, int distance) {
        ...
        if (this.currentPath != null && !this.currentPath.isFinished() && positions.contains(this.targetPos)) {
    ...
    protected void pathFollow() {
        ...
        double d0 = Math.abs(this.entity.getPosX() - ((double)vector3i.getX() + 0.5));
        double d1 = Math.abs(this.entity.getPosY() - (double)vector3i.getY());
        double d2 = Math.abs(this.entity.getPosZ() - ((double)vector3i.getZ() + 0.5));
    ...
    private boolean func_234112_b_(Vector3d currentPosition) {
        ...
        if (!currentPosition.isWithinDistanceOf(vector3d, 2.0)) {
            return false;
        }
        ...
        return vector3d2.dotProduct(vector3d3 = currentPosition.subtract(vector3d)) > 0.0;
    }
    ...
    protected void checkForStuck(Vector3d positionVec3) {
        ...
        if (this.currentPath != null && !this.currentPath.isFinished()) {
            ...
            if (vector3i.equals(this.timeoutCachedNode)) {
                ...
                double d = this.timeoutLimit = this.entity.getAIMoveSpeed() > 0.0f ? d0 / (double)this.entity.getAIMoveSpeed() * 1000.0 : 0.0;
            }
    ...
    protected void trimPath() {
        if (this.currentPath != null) {
            for (int i = 0; i < this.currentPath.getCurrentPathLength(); ++i) {
                ...
                if (pathpoint1 == null || pathpoint.y < pathpoint1.y) continue;
