...
extends NodeProcessor {
    ...
    public FlaggedPathPoint func_224768_a(double p_224768_1_, double p_224768_3_, double p_224768_5_) {
        return new FlaggedPathPoint(super.openPoint(MathHelper.floor(p_224768_1_ - (double)(this.entity.getWidth() / 2.0f)), MathHelper.floor(p_224768_3_ + 0.5), MathHelper.floor(p_224768_5_ - (double)(this.entity.getWidth() / 2.0f))));
    }
    ...
    public int func_222859_a(PathPoint[] p_222859_1_, PathPoint p_222859_2_) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (pathpoint == null || pathpoint.visited) continue;
        ...
        for (int i = p_186327_1_; i < p_186327_1_ + this.entitySizeX; ++i) {
            for (int j = p_186327_2_; j < p_186327_2_ + this.entitySizeY; ++j) {
                for (int k = p_186327_3_; k < p_186327_3_ + this.entitySizeZ; ++k) {
                    ...
                    if (fluidstate.isEmpty() && blockstate.allowsMovement(this.blockaccess, (BlockPos)blockpos$mutable.down(), PathType.WATER) && blockstate.isAir()) {
                        return PathNodeType.BREACH;
                    }
