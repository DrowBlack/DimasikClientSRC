...
extends Scoreboard {
    ...
    public void onScoreChanged(Score scoreIn) {
        ...
        if (this.addedObjectives.contains(scoreIn.getObjective())) {
            this.server.getPlayerList().sendPacketToAllPlayers(new SUpdateScorePacket(Action.CHANGE, scoreIn.getObjective().getName(), scoreIn.getPlayerName(), scoreIn.getScorePoints()));
        }
    ...
    public void onPlayerRemoved(String scoreName) {
        ...
        this.server.getPlayerList().sendPacketToAllPlayers(new SUpdateScorePacket(Action.REMOVE, null, scoreName, 0));
        this.markSaveDataDirty();
    }
    ...
    public void onPlayerScoreRemoved(String scoreName, ScoreObjective objective) {
        ...
        if (this.addedObjectives.contains(objective)) {
            this.server.getPlayerList().sendPacketToAllPlayers(new SUpdateScorePacket(Action.REMOVE, objective.getName(), scoreName, 0));
        }
        this.markSaveDataDirty();
    }
    ...
    public List<IPacket<?>> getCreatePackets(ScoreObjective objective) {
        ArrayList<IPacket<?>> list = Lists.newArrayList();
        ...
        for (Score score : this.getSortedScores(objective)) {
            list.add(new SUpdateScorePacket(Action.CHANGE, score.getObjective().getName(), score.getPlayerName(), score.getScorePoints()));
        }
        return list;
    }
