    ...
    public void run() {
        while (this.server.isServerRunning()) {
            ...
            if (k > this.maxTickTime) {
                ...
                crashreportcategory1.addDetail("Random tick rate", () -> this.server.func_240793_aU_().getGameRulesInstance().get(GameRules.RANDOM_TICK_SPEED).toString());
                crashreportcategory1.addDetail("Level stats", () -> Streams.stream(this.server.getWorlds()).map(p_244716_0_ -> String.valueOf(p_244716_0_.getDimensionKey()) + ": " + p_244716_0_.func_244521_F()).collect(Collectors.joining(",\n")));
