...
extends EnumProperty<Direction> {
    ...
    public static DirectionProperty create(String name, Predicate<Direction> filter) {
        return DirectionProperty.create(name, Arrays.stream(Direction.values()).filter(filter).collect(Collectors.toList()));
    }

    public static DirectionProperty create(String p_196962_0_, Direction ... p_196962_1_) {
        return DirectionProperty.create(p_196962_0_, Lists.newArrayList(p_196962_1_));
    }
