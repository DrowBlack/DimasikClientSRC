...
implements INameable {
    ...
    public void loadFromItemStack(ItemStack stack, DyeColor color) {
        this.patterns = BannerTileEntity.getPatternData(stack);
    ...
    public static void removeBannerData(ItemStack stack) {
        ListNBT listnbt;
        CompoundNBT compoundnbt = stack.getChildTag("BlockEntityTag");
        if (compoundnbt != null && compoundnbt.contains("Patterns", 9) && !(listnbt = compoundnbt.getList("Patterns", 10)).isEmpty()) {
    ...
    public ItemStack getItem(BlockState state) {
        ItemStack itemstack = new ItemStack(BannerBlock.forColor(this.getBaseColor(() -> state)));
