...
implements ITickableTileEntity {
    ...
    public void angerBees(@Nullable PlayerEntity p_226963_1_, BlockState p_226963_2_, State p_226963_3_) {
        List<Entity> list = this.tryReleaseBee(p_226963_2_, p_226963_3_);
        if (p_226963_1_ != null) {
            for (Entity entity : list) {
                if (!(entity instanceof BeeEntity)) continue;
                BeeEntity beeentity = (BeeEntity)entity;
                if (!(p_226963_1_.getPositionVec().squareDistanceTo(entity.getPositionVec()) <= 16.0)) continue;
                if (!this.isSmoked()) {
    ...
    private List<Entity> tryReleaseBee(BlockState p_226965_1_, State p_226965_2_) {
        ArrayList<Entity> list = Lists.newArrayList();
        this.bees.removeIf(p_226966_4_ -> this.func_235651_a_(p_226965_1_, (Bee)p_226966_4_, (List<Entity>)list, p_226965_2_));
        return list;
    }
    ...
    public void tryEnterHive(Entity p_226962_1_, boolean p_226962_2_, int p_226962_3_) {
        if (this.bees.size() < 3) {
            ...
            if (this.world != null) {
                BeeEntity beeentity;
                if (p_226962_1_ instanceof BeeEntity && (beeentity = (BeeEntity)p_226962_1_).hasFlower() && (!this.hasFlowerPos() || this.world.rand.nextBoolean())) {
    ...
    private boolean func_235651_a_(BlockState p_235651_1_, Bee p_235651_2_, @Nullable List<Entity> p_235651_3_, State p_235651_4_) {
        boolean flag;
        if ((this.world.isNightTime() || this.world.isRaining()) && p_235651_4_ != State.EMERGENCY) {
            return false;
        }
        BlockPos blockpos = this.getPos();
        ...
        boolean bl = flag = !this.world.getBlockState(blockpos1).getCollisionShape(this.world, blockpos1).isEmpty();
        ...
        if (entity != null) {
            ...
            if (entity instanceof BeeEntity) {
                ...
                if (p_235651_4_ == State.HONEY_DELIVERED) {
                    ...
                    if (p_235651_1_.getBlock().isIn(BlockTags.BEEHIVES) && (i = BeehiveTileEntity.getHoneyLevel(p_235651_1_)) < 5) {
                        int j;
                        int n = j = this.world.rand.nextInt(100) == 0 ? 2 : 1;
                        if (i + j > 5) {
                            --j;
                        }
                        this.world.setBlockState(this.getPos(), (BlockState)p_235651_1_.with(BeehiveBlock.HONEY_LEVEL, i + j));
                    }
                }
                ...
                double d0 = (double)blockpos.getX() + 0.5 + d3 * (double)direction.getXOffset();
                double d1 = (double)blockpos.getY() + 0.5 - (double)(entity.getHeight() / 2.0f);
                double d2 = (double)blockpos.getZ() + 0.5 + d3 * (double)direction.getZOffset();
    ...
    private void tickBees() {
        ...
        while (iterator.hasNext()) {
            ...
            if (beehivetileentity$bee.ticksInHive > beehivetileentity$bee.minOccupationTicks) {
                ...
                State state = beehivetileentity$state = beehivetileentity$bee.entityData.getBoolean("HasNectar") ? State.HONEY_DELIVERED : State.BEE_RELEASED;
    ...
    public void read(BlockState state, CompoundNBT nbt) {
        ...
        for (int i = 0; i < listnbt.size(); ++i) {
            ...
            Bee beehivetileentity$bee = new Bee(compoundnbt.getCompound("EntityData"), compoundnbt.getInt("TicksInHive"), compoundnbt.getInt("MinOccupationTicks"));
