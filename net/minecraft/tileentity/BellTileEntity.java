...
implements ITickableTileEntity {
    ...
    private void addRaiderParticles(World p_222826_1_) {
        if (p_222826_1_.isRemote) {
            ...
            int i = (int)this.entitiesAtRing.stream().filter(p_222829_1_ -> blockpos.withinDistance(p_222829_1_.getPositionVec(), 48.0)).count();
            this.entitiesAtRing.stream().filter(this::isNearbyRaider).forEach(p_235655_4_ -> {
                ...
                for (int k = 0; k < j; ++k) {
                    ...
                    double d2 = (double)ColorHelper.PackedColor.getRed(l) / 255.0;
                    double d3 = (double)ColorHelper.PackedColor.getGreen(l) / 255.0;
                    double d4 = (double)ColorHelper.PackedColor.getBlue(l) / 255.0;
    ...
    private boolean isNearbyRaider(LivingEntity p_222832_1_) {
        return p_222832_1_.isAlive() && !p_222832_1_.removed && this.getPos().withinDistance(p_222832_1_.getPositionVec(), 48.0) && p_222832_1_.getType().isContained(EntityTypeTags.RAIDERS);
    }
