...
implements ITickableTileEntity {
    ...
    public void read(BlockState state, CompoundNBT nbt) {
        ...
        this.targetUuid = nbt.hasUniqueId("Target") ? nbt.getUniqueId("Target") : null;
    }
    ...
    private void addEffectsToPlayers() {
        ...
        int j = i / 7 * 16;
        int k = this.pos.getX();
        AxisAlignedBB axisalignedbb = new AxisAlignedBB(k, l = this.pos.getY(), i1 = this.pos.getZ(), k + 1, l + 1, i1 + 1).grow(j).expand(0.0, this.world.getHeight(), 0.0);
        List<PlayerEntity> list = this.world.getEntitiesWithinAABB(PlayerEntity.class, axisalignedbb);
        if (!list.isEmpty()) {
            for (PlayerEntity playerentity : list) {
                if (!this.pos.withinDistance(playerentity.getPosition(), (double)j) || !playerentity.isWet()) continue;
    ...
    private void attackMobs() {
        ...
        if (i < 42) {
            ...
            List<LivingEntity> list = this.world.getEntitiesWithinAABB(LivingEntity.class, this.getAreaOfEffect(), p_205033_0_ -> p_205033_0_ instanceof IMob && p_205033_0_.isWet());
            if (!list.isEmpty()) {
    ...
    private AxisAlignedBB getAreaOfEffect() {
        ...
        int j = this.pos.getY();
        int k = this.pos.getZ();
        return new AxisAlignedBB(i, j, k, i + 1, j + 1, k + 1).grow(8.0);
    }

    @Nullable
    private LivingEntity findExistingTarget() {
        List<LivingEntity> list = this.world.getEntitiesWithinAABB(LivingEntity.class, this.getAreaOfEffect(), p_205032_1_ -> p_205032_1_.getUniqueID().equals(this.targetUuid));
        return list.size() == 1 ? list.get(0) : null;
    }

    private void spawnParticles() {
        Random random = this.world.rand;
        double d0 = MathHelper.sin((float)(this.ticksExisted + 35) * 0.1f) / 2.0f + 0.5f;
        d0 = (d0 * d0 + d0) * (double)0.3f;
        Vector3d vector3d = new Vector3d((double)this.pos.getX() + 0.5, (double)this.pos.getY() + 1.5 + d0, (double)this.pos.getZ() + 0.5);
        ...
        if (this.target != null) {
            ...
            Vector3d vector3d3 = new Vector3d(f3, f4, f5);
