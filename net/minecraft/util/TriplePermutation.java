...
public enum TriplePermutation {
    ...
    static {
        field_239185_i_ = Util.make(new TriplePermutation[TriplePermutation.values().length][TriplePermutation.values().length], p_239190_0_ -> {
            for (TriplePermutation triplepermutation : TriplePermutation.values()) {
                for (TriplePermutation triplepermutation1 : TriplePermutation.values()) {
                    TriplePermutation triplepermutation2;
                    int[] aint = new int[3];
                    for (int i = 0; i < 3; ++i) {
                        aint[i] = triplepermutation.field_239183_g_[triplepermutation1.field_239183_g_[i]];
                    }
                    p_239190_0_[triplepermutation.ordinal()][triplepermutation1.ordinal()] = triplepermutation2 = Arrays.stream(TriplePermutation.values()).filter(p_239189_1_ -> Arrays.equals(p_239189_1_.field_239183_g_, aint)).findFirst().get();
                }
            }
        });
    }
