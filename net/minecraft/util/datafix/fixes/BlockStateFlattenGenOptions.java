...
extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("LevelFlatGeneratorInfoFix", this.getInputSchema().getType(TypeReferences.LEVEL), p_207414_1_ -> p_207414_1_.update(DSL.remainderFinder(), this::fix));
    }

    private Dynamic<?> fix(Dynamic<?> p_209636_1_) {
        return p_209636_1_.get("generatorName").asString("").equalsIgnoreCase("flat") ? p_209636_1_.update("generatorOptions", p_209634_1_ -> DataFixUtils.orElse(p_209634_1_.asString().map(this::fixString).map(s -> p_209634_1_.createString((String)s)).result(), p_209634_1_)) : p_209636_1_;
    }
    ...
    String fixString(String p_199180_1_) {
        ...
        if (i >= 0 && i <= 3) {
            ...
            stringbuilder.append(StreamSupport.stream(field_199182_b.split(s1).spliterator(), false).map(p_206368_2_ -> {
                String s2;
                int j;
                List<String> list = splitter.splitToList((CharSequence)p_206368_2_);
                if (list.size() == 2) {
                ...
                return (String)(j == 1 ? "" : j + "*") + BlockStateFlatteningMap.getFixedNBTForID(l << 4 | j1).get("Name").asString("");
