...
extends DataFix {
    ...
    private static <T> Dynamic<T> fixTag(Dynamic<T> p_211926_0_) {
        boolean flag = p_211926_0_.get("Children").asStreamOpt().map(p_211928_0_ -> p_211928_0_.allMatch(IglooMetadataRemoval::isIglooPiece)).result().orElse(false);
    ...
    private static <T> Dynamic<T> removeIglooPieces(Dynamic<T> p_211929_0_) {
        return p_211929_0_.asStreamOpt().map(p_211925_0_ -> p_211925_0_.filter(p_211927_0_ -> !IglooMetadataRemoval.isIglooPiece(p_211927_0_))).map(p_211929_0_::createList).result().orElse(p_211929_0_);
    }
