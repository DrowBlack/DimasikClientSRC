...
public abstract class VoxelShapePart {
    ...
    public boolean isEmpty() {
        for (Direction.Axis direction$axis : AXIS_VALUES) {
            if (this.getStart(direction$axis) < this.getEnd(direction$axis)) continue;
            return true;
        }
        return false;
    }
    ...
    public int lastFilled(Direction.Axis axis, int p_197836_2_, int p_197836_3_) {
        if (p_197836_2_ >= 0 && p_197836_3_ >= 0) {
            ...
            if (p_197836_2_ < this.getSize(direction$axis) && p_197836_3_ < this.getSize(direction$axis1)) {
                ...
                for (int j = i - 1; j >= 0; --j) {
                    if (!this.isFilledWithRotation(axisrotation, j, p_197836_2_, p_197836_3_)) continue;
                    return j + 1;
                }
                return 0;
            }
    ...
    private void forEachEdgeOnAxis(ILineConsumer lineConsumer, AxisRotation axis, boolean p_197832_3_) {
                ...
                for (int k1 = 0; k1 <= l; ++k1) {
                    ...
                    for (int j2 = 0; j2 <= 1; ++j2) {
                        for (int k2 = 0; k2 <= 1; ++k2) {
                            if (!this.containsWithRotation(axisrotation, i1 + j2 - 1, j1 + k2 - 1, k1)) continue;
                            ++l1;
                            i2 ^= j2 ^ k2;
                        }
                    }
    ...
    protected boolean isZAxisLineFull(int fromZ, int toZ, int x, int y) {
        for (int i = fromZ; i < toZ; ++i) {
            if (this.contains(x, y, i)) continue;
            return false;
        }
        return true;
    }
    ...
    protected boolean isXZRectangleFull(int fromX, int toX, int fromZ, int toZ, int x) {
        for (int i = fromX; i < toX; ++i) {
            if (this.isZAxisLineFull(fromZ, toZ, i, x)) continue;
            return false;
        }
        return true;
    }
    ...
    public void forEachBox(ILineConsumer consumer, boolean combine) {
        BitSetVoxelShapePart voxelshapepart = new BitSetVoxelShapePart(this);
        for (int i = 0; i <= this.xSize; ++i) {
            for (int j = 0; j <= this.ySize; ++j) {
                ...
                for (int l = 0; l <= this.zSize; ++l) {
                    ...
                    int k1 = j;
                    int l1 = j;
                    ((VoxelShapePart)voxelshapepart).setZAxisLine(k, l, i, j, false);
                    while (((VoxelShapePart)voxelshapepart).isZAxisLineFull(k, l, i1 - 1, k1)) {
                        ((VoxelShapePart)voxelshapepart).setZAxisLine(k, l, i1 - 1, k1, false);
                        --i1;
                    }
                    while (((VoxelShapePart)voxelshapepart).isZAxisLineFull(k, l, j1 + 1, k1)) {
                        ((VoxelShapePart)voxelshapepart).setZAxisLine(k, l, j1 + 1, k1, false);
                        ++j1;
                    }
                    while (voxelshapepart.isXZRectangleFull(i1, j1 + 1, k, l, k1 - 1)) {
                        for (int i2 = i1; i2 <= j1; ++i2) {
                            ((VoxelShapePart)voxelshapepart).setZAxisLine(k, l, i2, k1 - 1, false);
                        }
                        --k1;
                    }
                    while (voxelshapepart.isXZRectangleFull(i1, j1 + 1, k, l, l1 + 1)) {
                        for (int j2 = i1; j2 <= j1; ++j2) {
                            ((VoxelShapePart)voxelshapepart).setZAxisLine(k, l, j2, l1 + 1, false);
                        }
                        ++l1;
                    }
    ...
    public void forEachFace(IFaceConsumer faceConsumer) {
    ...
    private void forEachFaceOnAxis(IFaceConsumer faceConsumer, AxisRotation axisRotationIn) {
        ...
        for (int l = 0; l < i; ++l) {
            for (int i1 = 0; i1 < j; ++i1) {
                ...
                for (int j1 = 0; j1 <= k; ++j1) {
                    boolean flag1;
                    boolean bl = flag1 = j1 != k && this.isFilledWithRotation(axisrotation, l, i1, j1);
                    if (!flag && flag1) {
