...
public enum Orientation implements IStringSerializable
{
    ...
    static {
        field_235524_ad_ = Util.make(new Orientation[Orientation.values().length][Orientation.values().length], p_235532_0_ -> {
            Map<Pair, Orientation> map = Arrays.stream(Orientation.values()).collect(Collectors.toMap(p_235536_0_ -> Pair.of(p_235536_0_.field_235523_ac_, p_235536_0_.func_235533_b_()), p_235535_0_ -> p_235535_0_));
            for (Orientation orientation : Orientation.values()) {
                for (Orientation orientation1 : Orientation.values()) {
        ...
        field_235525_ae_ = (Orientation[])Arrays.stream(Orientation.values()).map(p_235534_0_ -> Arrays.stream(Orientation.values()).filter(p_235528_1_ -> p_235534_0_.func_235527_a_((Orientation)p_235528_1_) == IDENTITY).findAny().get()).toArray(Orientation[]::new);
    }
}
