...
implements Comparable<Vector3i> {
    public static final Codec<Vector3i> CODEC = Codec.INT_STREAM.comapFlatMap(stream -> Util.validateIntStreamSize(stream, 3).map(componentArray -> new Vector3i(componentArray[0], componentArray[1], componentArray[2])), vector -> IntStream.of(vector.getX(), vector.getY(), vector.getZ()));
