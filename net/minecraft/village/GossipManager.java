...
public class GossipManager {
    ...
    public void transferFrom(GossipManager gossip, Random rand, int gossipAmount) {
        ...
        collection.forEach(gossipEntry -> {
            ...
            if (i >= 2) {
                this.getOrCreate((UUID)gossipEntry.target).gossipTypeMap.mergeInt(gossipEntry.type, i, GossipManager::getMax);
            }
        });
    }
    ...
    public void add(UUID identifier, GossipType gossipType, int gossipValue) {
        ...
        gossipmanager$gossips.gossipTypeMap.mergeInt(gossipType, gossipValue, (p_220915_2_, p_220915_3_) -> this.mergeValuesForAddition(gossipType, (int)p_220915_2_, (int)p_220915_3_));
    ...
    public <T> Dynamic<T> write(DynamicOps<T> dynamic) {
        return new Dynamic<Object>(dynamic, dynamic.createList(this.getGossipEntries().map(gossipEntry -> gossipEntry.write(dynamic)).map(Dynamic::getValue)));
    }

    public void read(Dynamic<?> dynamic) {
        dynamic.asStream().map(GossipEntry::read).flatMap(p_234056_0_ -> Util.streamOptional(p_234056_0_.result())).forEach(gossipEntry -> this.getOrCreate((UUID)gossipEntry.target).gossipTypeMap.put(gossipEntry.type, gossipEntry.value));
    }
    ...
    static class Gossips {
        ...
        public void decay() {
            Iterator objectiterator = this.gossipTypeMap.object2IntEntrySet().iterator();
    ...
    static class GossipEntry {
        ...
        public <T> Dynamic<T> write(DynamicOps<T> dynamic) {
            return new Dynamic<T>(dynamic, dynamic.createMap(ImmutableMap.of(dynamic.createString("Target"), UUIDCodec.CODEC.encodeStart(dynamic, this.target).result().orElseThrow(RuntimeException::new), dynamic.createString("Type"), dynamic.createString(this.type.id), dynamic.createString("Value"), dynamic.createInt(this.value))));
        }
