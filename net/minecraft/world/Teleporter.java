...
public class Teleporter {
    ...
    public Optional<TeleportationRepositioner.Result> getExistingPortal(BlockPos pos, boolean isNether) {
        ...
        Optional<PointOfInterest> optional = pointofinterestmanager.getInSquare(poiType -> poiType == PointOfInterestType.NETHER_PORTAL, pos, i, PointOfInterestManager.Status.ANY).sorted(Comparator.comparingDouble(poi -> poi.getPos().distanceSq(pos)).thenComparingInt(poi -> poi.getPos().getY())).filter(poi -> this.world.getBlockState(poi.getPos()).hasProperty(BlockStateProperties.HORIZONTAL_AXIS)).findFirst();
        return optional.map(poi -> {
            BlockPos blockpos = poi.getPos();
            this.world.getChunkProvider().registerTicket(TicketType.PORTAL, new ChunkPos(blockpos), 3, blockpos);
            BlockState blockstate = this.world.getBlockState(blockpos);
            return TeleportationRepositioner.findLargestRectangle(blockpos, blockstate.get(BlockStateProperties.HORIZONTAL_AXIS), 21, Direction.Axis.Y, 21, posIn -> this.world.getBlockState((BlockPos)posIn) == blockstate);
        });
    }
    ...
    public Optional<TeleportationRepositioner.Result> makePortal(BlockPos pos, Direction.Axis axis) {
        ...
        for (BlockPos.Mutable blockpos$mutable1 : BlockPos.func_243514_a(pos, 16, Direction.EAST, Direction.SOUTH)) {
            ...
            boolean k = true;
            if (!worldborder.contains(blockpos$mutable1) || !worldborder.contains(blockpos$mutable1.move(direction, 1))) continue;
            blockpos$mutable1.move(direction.getOpposite(), 1);
            for (int l = j; l >= 0; --l) {
                int j1;
                blockpos$mutable1.setY(l);
                if (!this.world.isAirBlock(blockpos$mutable1)) continue;
                int i1 = l;
                while (l > 0 && this.world.isAirBlock(blockpos$mutable1.move(Direction.DOWN))) {
                    --l;
                }
                if (l + 4 > i || (j1 = i1 - l) > 0 && j1 < 3) continue;
                blockpos$mutable1.setY(l);
                if (!this.checkRegionForPlacement(blockpos$mutable1, blockpos$mutable, direction, 0)) continue;
                double d2 = pos.distanceSq(blockpos$mutable1);
                if (this.checkRegionForPlacement(blockpos$mutable1, blockpos$mutable, direction, -1) && this.checkRegionForPlacement(blockpos$mutable1, blockpos$mutable, direction, 1) && (d0 == -1.0 || d0 > d2)) {
                    d0 = d2;
    ...
    private boolean checkRegionForPlacement(BlockPos originalPos, BlockPos.Mutable offsetPos, Direction directionIn, int offsetScale) {
        ...
        for (int i = -1; i < 3; ++i) {
            for (int j = -1; j < 4; ++j) {
                ...
                if (j < 0 || this.world.isAirBlock(offsetPos)) continue;
                return false;
            }
        }
        return true;
