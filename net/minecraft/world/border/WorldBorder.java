...
public class WorldBorder {
    ...
    private IBorderInfo state = new StationaryBorderInfo(6.0E7);
    ...
    implements IBorderInfo {
        ...
        private void updateBox() {
            this.minX = Math.max(WorldBorder.this.getCenterX() - this.size / 2.0, (double)(-WorldBorder.this.worldSize));
            this.minZ = Math.max(WorldBorder.this.getCenterZ() - this.size / 2.0, (double)(-WorldBorder.this.worldSize));
            this.maxX = Math.min(WorldBorder.this.getCenterX() + this.size / 2.0, (double)WorldBorder.this.worldSize);
            this.maxZ = Math.min(WorldBorder.this.getCenterZ() + this.size / 2.0, (double)WorldBorder.this.worldSize);
    ...
    implements IBorderInfo {
        ...
        @Override
        public double getMinX() {
            return Math.max(WorldBorder.this.getCenterX() - this.getSize() / 2.0, (double)(-WorldBorder.this.worldSize));
        }

        @Override
        public double getMinZ() {
            return Math.max(WorldBorder.this.getCenterZ() - this.getSize() / 2.0, (double)(-WorldBorder.this.worldSize));
        }

        @Override
        public double getMaxX() {
            return Math.min(WorldBorder.this.getCenterX() + this.getSize() / 2.0, (double)WorldBorder.this.worldSize);
        }

        @Override
        public double getMaxZ() {
            return Math.min(WorldBorder.this.getCenterZ() + this.getSize() / 2.0, (double)WorldBorder.this.worldSize);
        }

        @Override
        public double getSize() {
            double d0 = (double)(Util.milliTime() - this.startTime) / this.transitionTime;
            return d0 < 1.0 ? MathHelper.lerp(d0, this.oldSize, this.newSize) : this.newSize;
        }
        ...
        public IBorderInfo tick() {
            ...
            if (this.getTimeUntilTarget() <= 0L) {
                ...
                iBorderInfo = worldBorder.new StationaryBorderInfo(this.newSize);
            } else {
                iBorderInfo = this;
            }
            return iBorderInfo;
    ...
    public static class Serializer {
        ...
        public static Serializer deserialize(DynamicLike<?> dynamic, Serializer defaultIn) {
        ...
        public void serialize(CompoundNBT nbt) {
            ...
            nbt.putDouble("BorderWarningBlocks", this.warningDistance);
