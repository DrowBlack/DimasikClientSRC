...
public class StructureManager {
    ...
    public StructureManager func_241464_a_(WorldGenRegion p_241464_1_) {
        if (p_241464_1_.getWorld() != this.field_235003_a_) {
            throw new IllegalStateException("Using invalid feature manager (source level: " + String.valueOf(p_241464_1_.getWorld()) + ", region: " + String.valueOf(p_241464_1_));
        }
    ...
    public Stream<? extends StructureStart<?>> func_235011_a_(SectionPos p_235011_1_, Structure<?> p_235011_2_) {
        return this.field_235003_a_.getChunk(p_235011_1_.getSectionX(), p_235011_1_.getSectionZ(), ChunkStatus.STRUCTURE_REFERENCES).func_230346_b_(p_235011_2_).stream().map(p_235015_0_ -> SectionPos.from(new ChunkPos((long)p_235015_0_), 0)).map(p_235006_2_ -> this.func_235013_a_((SectionPos)p_235006_2_, p_235011_2_, this.field_235003_a_.getChunk(p_235006_2_.getSectionX(), p_235006_2_.getSectionZ(), ChunkStatus.STRUCTURE_STARTS))).filter(p_235007_0_ -> p_235007_0_ != null && p_235007_0_.isValid());
    }
    ...
    public StructureStart<?> func_235010_a_(BlockPos p_235010_1_, boolean p_235010_2_, Structure<?> p_235010_3_) {
        return DataFixUtils.orElse(this.func_235011_a_(SectionPos.from(p_235010_1_), p_235010_3_).filter(p_235009_1_ -> p_235009_1_.getBoundingBox().isVecInside(p_235010_1_)).filter(p_235016_2_ -> !p_235010_2_ || p_235016_2_.getComponents().stream().anyMatch(p_235008_1_ -> p_235008_1_.getBoundingBox().isVecInside(p_235010_1_))).findFirst(), StructureStart.DUMMY);
    }
}
