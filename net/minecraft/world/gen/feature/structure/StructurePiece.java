...
public abstract class StructurePiece {
    ...
    public StructurePiece(IStructurePieceType structurePierceTypeIn, CompoundNBT nbt) {
        ...
        this.setCoordBaseMode((i = nbt.getInt("O")) == -1 ? null : Direction.byHorizontalIndex(i));
    }
    ...
    public static StructurePiece findIntersecting(List<StructurePiece> listIn, MutableBoundingBox boundingboxIn) {
        for (StructurePiece structurepiece : listIn) {
            if (structurepiece.getBoundingBox() == null || !structurepiece.getBoundingBox().intersectsWith(boundingboxIn)) continue;
    ...
    protected boolean isLiquidInStructureBoundingBox(IBlockReader worldIn, MutableBoundingBox boundingboxIn) {
        ...
        for (int k1 = i; k1 <= l; ++k1) {
            for (int l1 = k; l1 <= j1; ++l1) {
                ...
                if (!worldIn.getBlockState(blockpos$mutable.setPos(k1, i1, l1)).getMaterial().isLiquid()) continue;
                return true;
            }
        }
        for (int i2 = i; i2 <= l; ++i2) {
            for (int k2 = j; k2 <= i1; ++k2) {
                ...
                if (!worldIn.getBlockState(blockpos$mutable.setPos(i2, k2, j1)).getMaterial().isLiquid()) continue;
                return true;
            }
        }
        for (int j2 = k; j2 <= j1; ++j2) {
            for (int l2 = j; l2 <= i1; ++l2) {
                ...
                if (!worldIn.getBlockState(blockpos$mutable.setPos(l, l2, j2)).getMaterial().isLiquid()) continue;
                return true;
            }
        }
        return false;
    ...
    protected boolean getSkyBrightness(IWorldReader worldIn, int x, int y, int z, MutableBoundingBox boundingboxIn) {
        ...
        int j;
        int i = this.getXWithOffset(x, z);
        BlockPos blockpos = new BlockPos(i, j = this.getYWithOffset(y + 1), k = this.getZWithOffset(x, z));
        if (!boundingboxIn.isVecInside(blockpos)) {
            return false;
        }
        ...
        for (int i = yMin; i <= yMax; ++i) {
            for (int j = xMin; j <= xMax; ++j) {
                for (int k = zMin; k <= zMax; ++k) {
                    if (existingOnly && this.getBlockStateFromPos(worldIn, j, i, k, boundingboxIn).isAir()) continue;
    ...
    protected void fillWithRandomizedBlocks(ISeedReader worldIn, MutableBoundingBox boundingboxIn, int minX, int minY, int minZ, int maxX, int maxY, int maxZ, boolean alwaysReplace, Random rand, BlockSelector blockselector) {
        for (int i = minY; i <= maxY; ++i) {
            for (int j = minX; j <= maxX; ++j) {
                for (int k = minZ; k <= maxZ; ++k) {
                    if (alwaysReplace && this.getBlockStateFromPos(worldIn, j, i, k, boundingboxIn).isAir()) continue;
    ...
    protected void generateMaybeBox(ISeedReader worldIn, MutableBoundingBox sbb, Random rand, float chance, int x1, int y1, int z1, int x2, int y2, int z2, BlockState edgeState, BlockState state, boolean requireNonAir, boolean requiredSkylight) {
        for (int i = y1; i <= y2; ++i) {
            for (int j = x1; j <= x2; ++j) {
                for (int k = z1; k <= z2; ++k) {
                    if (rand.nextFloat() > chance || requireNonAir && this.getBlockStateFromPos(worldIn, j, i, k, sbb).isAir() || requiredSkylight && !this.getSkyBrightness(worldIn, j, i, k, sbb)) continue;
        ...
        for (int i = minY; i <= maxY; ++i) {
            ...
            for (int j = minX; j <= maxX; ++j) {
                ...
                for (int k = minZ; k <= maxZ; ++k) {
                    ...
                    if (excludeAir && this.getBlockStateFromPos(worldIn, j, i, k, boundingboxIn).isAir() || !((f8 = f6 * f6 + f5 * f5 + f7 * f7) <= 1.05f)) continue;
    ...
    public static BlockState correctFacing(IBlockReader worldIn, BlockPos posIn, BlockState blockStateIn) {
        ...
        for (Direction direction1 : Direction.Plane.HORIZONTAL) {
            ...
            if (!blockstate.isOpaqueCube(worldIn, blockpos)) continue;
            if (direction != null) {
                direction = null;
                break;
            }
        ...
        if (direction != null) {
            return (BlockState)blockStateIn.with(HorizontalBlock.HORIZONTAL_FACING, direction.getOpposite());
        }
        ...
        return (BlockState)blockStateIn.with(HorizontalBlock.HORIZONTAL_FACING, direction2);
    }
    ...
    protected boolean generateChest(IServerWorld worldIn, MutableBoundingBox boundsIn, Random rand, BlockPos posIn, ResourceLocation resourceLocationIn, @Nullable BlockState p_191080_6_) {
        if (boundsIn.isVecInside(posIn) && !worldIn.getBlockState(posIn).isIn(Blocks.CHEST)) {
            if (p_191080_6_ == null) {
                p_191080_6_ = StructurePiece.correctFacing(worldIn, posIn, Blocks.CHEST.getDefaultState());
            }
    ...
    protected boolean createDispenser(ISeedReader worldIn, MutableBoundingBox sbb, Random rand, int x, int y, int z, Direction facing, ResourceLocation lootTableIn) {
        ...
        if (sbb.isVecInside(blockpos) && !worldIn.getBlockState(blockpos).isIn(Blocks.DISPENSER)) {
            this.setBlockState(worldIn, (BlockState)Blocks.DISPENSER.getDefaultState().with(DispenserBlock.FACING, facing), x, y, z, sbb);
