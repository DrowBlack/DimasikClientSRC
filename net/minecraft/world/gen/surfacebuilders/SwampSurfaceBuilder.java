...
extends SurfaceBuilder<SurfaceBuilderConfig> {
    ...
    public void buildSurface(Random random, IChunk chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, long seed, SurfaceBuilderConfig config) {
        double d0 = Biome.INFO_NOISE.noiseAt((double)x * 0.25, (double)z * 0.25, false);
        if (d0 > 0.0) {
            int i = x & 0xF;
            int j = z & 0xF;
            ...
            for (int k = startHeight; k >= 0; --k) {
                ...
                if (chunkIn.getBlockState(blockpos$mutable).isAir()) continue;
                if (k != 62 || chunkIn.getBlockState(blockpos$mutable).isIn(defaultFluid.getBlock())) break;
