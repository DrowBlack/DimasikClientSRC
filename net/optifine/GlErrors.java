...
public class GlErrors {
    ...
    public static void frameStart() {
        ...
        if (!glErrors.isEmpty()) {
            ...
            if (System.currentTimeMillis() > timeCheckStartMs + 3000L) {
                for (GlError glerrors$glerror : glErrors.values()) {
    ...
    public static boolean isEnabled(int error) {
        ...
        GlError glerrors$glerror = GlErrors.getGlError(error);
        return glerrors$glerror.isEnabled();
    }

    private static GlError getGlError(int error) {
        GlError glerrors$glerror = glErrors.get(error);
        if (glerrors$glerror == null) {
            glerrors$glerror = new GlError(error);
