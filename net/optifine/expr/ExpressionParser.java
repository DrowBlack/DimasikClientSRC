...
public class ExpressionParser {
    ...
    public IExpressionFloat parseFloat(String str) throws ParseException {
        ...
        if (!(iexpression instanceof IExpressionFloat)) {
            throw new ParseException("Not a float expression: " + String.valueOf((Object)iexpression.getExpressionType()));
        }
    ...
    public IExpressionBool parseBool(String str) throws ParseException {
        ...
        if (!(iexpression instanceof IExpressionBool)) {
            throw new ParseException("Not a boolean expression: " + String.valueOf((Object)iexpression.getExpressionType()));
        }
    ...
    private IExpression parseInfix(Deque<Token> deque) throws ParseException {
        ...
        while ((token = deque.poll()) != null) {
            if (token.getType() != TokenType.OPERATOR) {
                throw new ParseException("Invalid operator: " + String.valueOf(token));
            }
    ...
    private IExpression makeInfix(List<IExpression> listExpr, List<Token> listOper) throws ParseException {
        ...
        for (Token token : listOper) {
            ...
            ExpressionParser.checkNull((Object)functiontype, "Invalid operator: " + String.valueOf(token));
    ...
    private IExpression parseExpression(Deque<Token> deque) throws ParseException {
        ...
        switch (token.getType()) {
            ...
            case OPERATOR: {
                ...
                ExpressionParser.checkNull((Object)functiontype1, "Invalid operator: " + String.valueOf(token));
    ...
    private FunctionType getFunctionType(Token token, Deque<Token> deque) throws ParseException {
        ...
        if (tokennext != null && tokennext.getType() == TokenType.BRACKET_OPEN) {
            FunctionType functiontype1 = FunctionType.parse(token.getText());
            ExpressionParser.checkNull((Object)functiontype1, "Unknown function: " + String.valueOf(token));
        ...
        if (functiontype.getParameterCount(new IExpression[0]) > 0) {
            throw new ParseException("Missing arguments: " + String.valueOf((Object)functiontype));
        }
        return functiontype;
    }
    ...
    private IExpression makeFunction(FunctionType type, Deque<Token> dequep) throws ParseException {
        Token token;
        if (type.getParameterCount(new IExpression[0]) == 0 && ((token = dequep.peek()) == null || token.getType() != TokenType.BRACKET_OPEN)) {
            return ExpressionParser.makeFunction(type, new IExpression[0]);
        }
        Token token1 = dequep.poll();
        Deque<Token> deque = ExpressionParser.getGroup(dequep, TokenType.BRACKET_CLOSE, true);
    ...
    private IExpression[] parseExpressions(Deque<Token> deque) throws ParseException {
        ...
        while ((iexpression = this.parseInfix(dequea = ExpressionParser.getGroup(deque, TokenType.COMMA, false))) != null) {
            list.add(iexpression);
        }
    ...
    private static IExpression makeFunction(FunctionType type, IExpression[] args) throws ParseException {
        ...
        for (int i = 0; i < args.length; ++i) {
            ...
            throw new ParseException("Invalid argument type, function: \"" + type.getName() + "\", index: " + i + ", type: " + String.valueOf((Object)expressiontype) + ", should be: " + String.valueOf((Object)expressiontype1));
        }
        ...
        throw new ParseException("Unknown function type: " + String.valueOf((Object)type.getExpressionType()) + ", function: " + type.getName());
    }
    ...
    private static Deque<Token> getGroup(Deque<Token> dequep, TokenType tokenTypeEnd, boolean tokenEndRequired) throws ParseException {
        ArrayDeque<Token> deque = new ArrayDeque<Token>();
        int i = 0;
        ...
        if (tokenEndRequired) {
            throw new ParseException("Missing end token: " + String.valueOf((Object)tokenTypeEnd));
        }
        return deque;
    }
