...
public class MacroProcessor {
    public static InputStream process(InputStream in, String path, boolean useShaderOptions) throws IOException {
        Object s = Config.readInputStream(in, "ASCII");
        String s1 = MacroProcessor.getMacroHeader((String)s, useShaderOptions);
        if (!s1.isEmpty()) {
            s = s1 + (String)s;
            if (Shaders.saveFinalShaders) {
                String s2 = path.replace(':', '/') + ".pre";
                Shaders.saveShader(s2, (String)s);
            }
            s = MacroProcessor.process((String)s);
        }
        if (Shaders.saveFinalShaders) {
            String s3 = path.replace(':', '/');
            Shaders.saveShader(s3, (String)s);
        }
        byte[] abyte = ((String)s).getBytes("ASCII");
    ...
    private static String getMacroHeader(String str, boolean useShaderOptions) throws IOException {
        ...
        block0: while (true) {
            ...
            if (useShaderOptions) {
                if (list == null) {
                    list = MacroProcessor.getMacroOptions();
                }
                iterator = list.iterator();
                while (iterator.hasNext()) {
                    shaderoption = iterator.next();
                    if (!s.contains(shaderoption.getName())) continue;
            ...
            while (true) {
                ...
                continue block0;
                shadermacro = (ShaderMacro)iterator1.next();
