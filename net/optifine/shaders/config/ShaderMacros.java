...
public class ShaderMacros {
    ...
    public static String getVendor() {
        ...
        if ((s1 = s1.toLowerCase()).startsWith("amd")) {
            return MC_GL_VENDOR_AMD;
        }
        if (s1.startsWith("ati")) {
            return MC_GL_VENDOR_ATI;
        }
        if (s1.startsWith("intel")) {
            return MC_GL_VENDOR_INTEL;
        }
        if (s1.startsWith("nvidia")) {
    ...
    public static ShaderMacro[] getConstantMacros() {
        if (constantMacros == null) {
            ...
            list.addAll(Arrays.asList(ShaderMacros.getRenderStages()));
    ...
    public static String getTextureMacroLines() {
        ...
        if (s != null) {
            ShaderMacros.addMacroLine(stringbuilder, MC_TEXTURE_FORMAT_ + s);
            String s1 = itextureformat.getMacroVersion();
            if (s1 != null) {
                ShaderMacros.addMacroLine(stringbuilder, MC_TEXTURE_FORMAT_ + s + "_" + s1);
            }
        }
        return stringbuilder.toString();
    }
    ...
    public static String[] getHeaderMacroLines() {
        String s = ShaderMacros.getFixedMacroLines() + ShaderMacros.getOptionMacroLines() + ShaderMacros.getTextureMacroLines();
