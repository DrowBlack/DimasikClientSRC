...
public class CacheObjectArray {
    ...
    public static void main(String[] args) throws Exception {
        ...
        CacheObjectArray.testNewObjDyn(BlockState.class, i, j);
        ...
        long k1 = CacheObjectArray.testNewObjDyn(BlockState.class, i, j);
        Config.dbg("New: " + k);
        Config.dbg("Clone: " + l);
    ...
    private static long testCloneObj(int size, int count) {
        ...
        for (int j = 0; j < count; ++j) {
            BlockState[] blockStateArray = (BlockState[])ablockstate.clone();
        }
        long k = System.currentTimeMillis();
        return k - i;
    }
    ...
    private static long testNewObj(int size, int count) {
        long i = System.currentTimeMillis();
        for (int j = 0; j < count; ++j) {
            BlockState[] blockStateArray = new BlockState[size];
        }
        long k = System.currentTimeMillis();
        return k - i;
    }
