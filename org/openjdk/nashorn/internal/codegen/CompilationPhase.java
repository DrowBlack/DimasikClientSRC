...
import org.openjdk.nashorn.internal.codegen.AssignSymbols;
import org.openjdk.nashorn.internal.codegen.CacheAst;
import org.openjdk.nashorn.internal.codegen.ClassEmitter;
import org.openjdk.nashorn.internal.codegen.CodeGenerator;
import org.openjdk.nashorn.internal.codegen.CompilationException;
...
import org.openjdk.nashorn.internal.codegen.FindScopeDepths;
import org.openjdk.nashorn.internal.codegen.FoldConstants;
import org.openjdk.nashorn.internal.codegen.LocalVariableTypesCalculator;
import org.openjdk.nashorn.internal.codegen.Lower;
import org.openjdk.nashorn.internal.codegen.OptimisticTypesCalculator;
import org.openjdk.nashorn.internal.codegen.ProgramPoints;
import org.openjdk.nashorn.internal.codegen.ReplaceCompileUnits;
import org.openjdk.nashorn.internal.codegen.SplitIntoFunctions;
import org.openjdk.nashorn.internal.codegen.Splitter;
...
abstract class CompilationPhase {
    ...
    extends CompilationPhase {
        ...
        FunctionNode transform(Compiler compiler, Compiler.CompilationPhases phases, FunctionNode fn) {
            ...
            if (log.isEnabled()) {
                log.fine("Installed class '" + rootClass.getSimpleName() + "' [" + rootClass.getName() + ", size=" + length + " bytes, " + compiler.getCompileUnits().size() + " compile unit(s)]");
            }
        ...
        FunctionNode transform(final Compiler compiler, final Compiler.CompilationPhases phases, FunctionNode fn) {
            ...
            FunctionNode newFn = CompilationPhase.transformFunction(fn, new ReplaceCompileUnits(){
                ...
                CompileUnit getReplacement(CompileUnit oldUnit) {
                    ...
                    return Objects.requireNonNullElseGet(existing, () -> this.createCompileUnit(oldUnit, unitSet, unitMap, compiler, phases));
                }
    ...
    extends CompilationPhase {
        ...
        FunctionNode transform(Compiler compiler, Compiler.CompilationPhases phases, FunctionNode fn) {
            ...
            log.fine("Done");
            return CompilationPhase.transformFunction(fn, new ReplaceCompileUnits(){
    ...
    extends CompilationPhase {
        ...
        @Override
        FunctionNode transform(Compiler compiler, Compiler.CompilationPhases phases, FunctionNode fn) {
            FunctionNode newFunctionNode = CompilationPhase.transformFunction(fn, new LocalVariableTypesCalculator(compiler, compiler.getReturnType()));
            ScriptEnvironment senv = compiler.getScriptEnvironment();
