...
extends SimpleNodeVisitor {
    ...
    public boolean enterReturnNode(ReturnNode returnNode) {
        ...
        if (returnExpr != null) {
            returnExprType = this.visitExpressionOnEmptyStack((Expression)returnExpr).type;
        } else {
            this.assertTypeStackIsEmpty();
    ...
    public boolean enterUnaryNode(UnaryNode unaryNode) {
        ...
        if (unaryNode.tokenType() == TokenType.DELETE && expr instanceof IdentNode) {
            unaryType = LocalVariableTypesCalculator.toLvarType(unaryNode.getType());
        } else {
            unaryType = LocalVariableTypesCalculator.toLvarType(unaryNode.setExpression(this.visitExpression((Expression)expr).typeExpression).getType());
        ...
        SimpleNodeVisitor applyChangesVisitor = new SimpleNodeVisitor(){
            ...
            public Node leaveIdentNode(IdentNode identNode) {
                ...
                if (symbol.hasSlot()) {
                    ...
                    LvarType lvarType = LocalVariableTypesCalculator.this.identifierLvarTypes.remove(original);
                    if (lvarType != null) {
