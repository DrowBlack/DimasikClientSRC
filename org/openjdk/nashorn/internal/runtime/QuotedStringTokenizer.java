...
public final class QuotedStringTokenizer {
    ...
    private QuotedStringTokenizer(String str, String delim, char[] quotes) {
        ...
        while (st.hasMoreTokens()) {
            ...
            while (this.unmatchedQuotesIn((String)token)) {
                if (!st.hasMoreTokens()) {
                    throw new IndexOutOfBoundsException((String)token);
                }
                token = (String)token + (delimIsWhitespace ? " " : delim) + st.nextToken();
            }
            this.tokens.add(this.stripQuotes((String)token));
        }
    }
