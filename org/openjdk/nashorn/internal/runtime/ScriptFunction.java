...
extends ScriptObject {
    ...
    protected GuardedInvocation findCallMethod(CallSiteDescriptor desc, LinkRequest request) {
        ...
        if (this.data.needsCallee()) {
            ...
            boundHandle = Lookup.MH.dropArguments(Lookup.MH.bindTo(callHandle, ScriptFunction.getLookupPrivileged(desc)), 0, new Class[]{type.parameterType(0), type.parameterType(1)});
        ...
        SwitchPoint[] spsArray = sps.isEmpty() ? null : sps.toArray(new SwitchPoint[0]);
    ...
    private static MethodHandles.Lookup getLookupPrivileged(CallSiteDescriptor desc) {
        return AccessController.doPrivileged(desc::getLookup, GET_LOOKUP_PERMISSION_CONTEXT);
    }
    ...
    private static MethodHandle bindToNameIfNeeded(MethodHandle methodHandle, String bindName) {
        ...
        boolean isVarArg = ((Class)methodType.parameterType(parameterCount - 1)).isArray();
        if (isVarArg) {
