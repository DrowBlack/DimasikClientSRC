...
implements Serializable {
    ...
    CompiledFunction bind(CompiledFunction originalInv, ScriptFunction fn, Object self, Object[] args) {
        ...
        if (this.isConstructor()) {
            return new CompiledFunction(boundInvoker, ScriptFunctionData.bindConstructHandle(originalInv.createComposableConstructor(), fn, args), null);
        }
    ...
    final CompiledFunction getBestConstructor(MethodType callSiteType, ScriptObject runtimeScope, Collection<CompiledFunction> forbidden) {
        ...
        return this.getBest(callSiteType.insertParameterTypes(1, Object.class), runtimeScope, forbidden);
    }
    ...
    CompiledFunction lookupExactApplyToCall(MethodType type) {
        MethodType adaptedType = this.needsCallee() ? type : type.dropParameterTypes(0, 1);
        for (CompiledFunction cf : this.code) {
            MethodType cftype;
            if (!cf.isApplyToCall() || (cftype = cf.type()).parameterCount() != adaptedType.parameterCount() || !ScriptFunctionData.widen(cftype).equals((Object)ScriptFunctionData.widen(adaptedType))) continue;
            return cf;
        }
        return null;
    }
