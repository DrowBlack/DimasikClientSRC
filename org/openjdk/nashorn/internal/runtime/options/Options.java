...
import org.openjdk.nashorn.internal.runtime.options.OptionTemplate;
...
public final class Options {
    ...
    private void processArgList(LinkedList<String> argList) {
        while (!argList.isEmpty()) {
            ...
            if (parg.template.isRepeated()) {
                ...
                String key = this.key(parg.template.getKey());
                String value = this.options.containsKey(key) ? this.options.get(key).getValue() + "," + parg.value : Objects.toString(parg.value);
                this.options.put(key, new Option<String>(value));
            } else {
    ...
    private static Option<?> createOption(OptionTemplate t, String value) {
        switch (t.getType()) {
            ...
            case "boolean": {
                return new Option<Boolean>(Boolean.parseBoolean(value));
            }
            case "integer": {
                try {
